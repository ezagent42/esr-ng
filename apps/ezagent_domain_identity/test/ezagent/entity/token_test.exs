defmodule Ezagent.Entity.TokenTest do
  ...
  describe "mint/2" do
    test "returns {plain_token, %EntityToken{}} for a valid entity URI" do
      uri = URI.parse("entity://agent/cc_mint-#{System.unique_integer([:positive])}")
    ...
    test "mint for a user URI also works" do
      uri = URI.parse("entity://user/token-#{System.unique_integer([:positive])}")
      assert {_plain, row} = Token.mint(uri)
      assert row.entity_uri == URI.to_string(uri)
    end

    test "multiple mints for same entity URI produce different tokens" do
      uri = URI.parse("entity://agent/cc_multi-#{System.unique_integer([:positive])}")
  ...
  describe "verify/2" do
    test "minted token verifies + returns {:ok, %{caps: caps}}" do
      uri = URI.parse("entity://agent/cc_verify-#{System.unique_integer([:positive])}")
    ...
    test "wrong token → {:error, :invalid_credentials}" do
      uri = URI.parse("entity://agent/cc_v2-#{System.unique_integer([:positive])}")
    ...
    test "no tokens for entity → {:error, :no_such_entity}" do
      uri = URI.parse("entity://agent/cc_v3-#{System.unique_integer([:positive])}")
    ...
    test "verify on wrong URI even with right plain string → {:error, :invalid_credentials}" do
      uri_a = URI.parse("entity://agent/cc_va-#{System.unique_integer([:positive])}")
      uri_b = URI.parse("entity://agent/cc_vb-#{System.unique_integer([:positive])}")
    ...
    test "verify updates last_used_at" do
      uri = URI.parse("entity://agent/cc_lu-#{System.unique_integer([:positive])}")
  ...
  describe "list/1" do
    test "returns rows for entity, sorted by created_at desc" do
      uri = URI.parse("entity://agent/cc_list-#{System.unique_integer([:positive])}")
    ...
    test "returns [] for entity with no tokens" do
      uri = URI.parse("entity://agent/cc_empty-#{System.unique_integer([:positive])}")
  ...
  describe "revoke/1" do
    test "revoked token no longer verifies" do
      uri = URI.parse("entity://agent/cc_revoke-#{System.unique_integer([:positive])}")
      {plain, row} = Token.mint(uri)
