defmodule EzagentPluginCc.BridgeRegistryTest do
  ...
  test "bind / lookup / unbind roundtrip" do
    uri = URI.new!("entity://agent/default/test_test-bridge-#{System.unique_integer([:positive])}")
  ...
  test "double-bind same pid is idempotent" do
    uri = URI.new!("entity://agent/default/test_test-bridge-double")
    pid = spawn(fn -> Process.sleep(:infinity) end)
  ...
  test "bind on top of live pid returns :already_bound" do
    uri = URI.new!("entity://agent/default/test_test-bridge-conflict")
  ...
  test "bind replaces a dead pid silently" do
    uri = URI.new!("entity://agent/default/test_test-bridge-replace")
    p1 = spawn(fn -> :ok end)
    Process.sleep(20)
  ...
  describe "PR 32a — observability surface" do
    test "count/0 reflects current bindings" do
      ...
      a = URI.new!("entity://agent/default/test_count-a")
      b = URI.new!("entity://agent/default/test_count-b")
      pid = spawn(fn -> Process.sleep(:infinity) end)
    ...
    test "status/0 reports :no_bridges or {:connected, n}" do
      ...
      uri = URI.new!("entity://agent/default/test_status-1")
    ...
    test "list_connected/0 carries info + connected_at" do
      uri = URI.new!("entity://agent/default/test_info-bridge")
      pid = spawn(fn -> Process.sleep(:infinity) end)
      ...
      assert URI.to_string(found_uri) == "entity://agent/default/test_info-bridge"
      assert row.pid == pid
    ...
    test "list_all/0 keeps {uri, pid} back-compat shape" do
      uri = URI.new!("entity://agent/default/test_list-all-bc")
      pid = spawn(fn -> Process.sleep(:infinity) end)
      :ok = BridgeRegistry.bind(uri, pid)

      matched =
        BridgeRegistry.list_all()
        |> Enum.find(fn {u, _} -> URI.to_string(u) == "entity://agent/default/test_list-all-bc" end)

      assert {%URI{}, ^pid} = matched
    end

    test "bind/unbind broadcast on topic/0" do
      uri = URI.new!("entity://agent/default/test_broadcast-bridge")
    ...
    test "unbind on absent uri is :ok and silent" do
      uri = URI.new!("entity://agent/default/test_never-bound")
      :ok = Phoenix.PubSub.subscribe(EzagentCore.PubSub, BridgeRegistry.topic())
