defmodule Ezagent.Template.FeishuChatBindingTest do
  @moduledoc """
  ...
  alias Ezagent.Template.FeishuChatBinding
  ...
  test "instantiate spawns Feishu Receiver Kind + adds session-scoped routing rule" do
    ...
    assert {:ok, _pid} = Ezagent.KindRegistry.lookup(feishu_uri)
    ...
    matched =
      Enum.any?(rules, fn row ->
        URI.to_string(feishu_uri) in row.receivers and
          row.matcher_data == Ezagent.Routing.Matcher.to_json(Ezagent.Routing.Matcher.in_session(session_uri))
      end)
    ...
    rules_after = Ezagent.Routing.RuleStore.list(EzagentDomainChat.Routing.MentionRouting)
  ...
  test "FeishuChat Kind type_name + uri_for + chat_id_from_uri" do
    assert Ezagent.Entity.FeishuChat.type_name() == :feishu_chat
    uri = Ezagent.Entity.FeishuChat.uri_for("oc_xyz")
    assert URI.to_string(uri) == "feishu://oc_xyz"
    assert Ezagent.Entity.FeishuChat.chat_id_from_uri(uri) == "oc_xyz"
  end
end
