defmodule Mix.Tasks.Esr.Plugin.InstallTest do
  @moduledoc """
  Phase 7 PR 36 invariant — `mix esr.plugin.install` exists and
  ...
  test "Mix.Tasks.Esr.Plugin.Install module exists with run/1" do
    Code.ensure_loaded(Mix.Tasks.Esr.Plugin.Install)

    assert function_exported?(Mix.Tasks.Esr.Plugin.Install, :run, 1),
           "expected run/1 to be defined on Mix.Tasks.Esr.Plugin.Install"
  end

  test "moduledoc covers the D7-8 contract surface" do
    source =
      File.read!(
        Path.join(__DIR__, "../../../lib/mix/tasks/esr.plugin.install.ex")
      )
