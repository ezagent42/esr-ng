defmodule Esr.Kind.RuntimePhase3dTest do
  @moduledoc """
  ...
  `{:error, :unauthorized}` + `[:esr, :authz, :denied]` telemetry.
  ...
  test "dispatch with empty caps → {:error, :unauthorized} + :denied telemetry" do
    ...
    assert_receive {:authz_event, [:esr, :authz, :denied], meta}, 500
  ...
  test "dispatch with admin caps → success + :granted telemetry" do
    ...
    assert_receive {:authz_event, [:esr, :authz, :granted], _meta}, 500
  end
