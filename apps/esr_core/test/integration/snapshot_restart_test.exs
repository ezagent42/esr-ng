defmodule Esr.Integration.SnapshotRestartTest do
  @moduledoc """
    ...
    test "User caps granted before restart are present after restart" do
      ...
      {:ok, pid1} =
        DynamicSupervisor.start_child(
          ...
          {Esr.Kind.Server,
           {Esr.Entity.User, %{uri: uri, initial_caps: caps}}}
        )
      ...
      :ok = Snapshot.save_now(uri, Esr.Entity.User, slice_with_caps)
      ...
      :ok = DynamicSupervisor.terminate_child(Esr.Workspace.Supervisor, pid1)
      ...
      {:ok, pid2} =
        DynamicSupervisor.start_child(
          Esr.Workspace.Supervisor,
          {Esr.Kind.Server, {Esr.Entity.User, %{uri: uri}}}
        )
  ...
  describe ":ephemeral does NOT persist" do
    test "TestKind state lost on restart" do
      ...
      {:ok, pid1} =
        DynamicSupervisor.start_child(
          Esr.Workspace.Supervisor,
          {Esr.Kind.Server, {Esr.Test.TestKind, %{uri: uri}}}
        )
