defmodule Mix.Tasks.Esr.Bootstrap do
  ...
  ## Usage
      ...
      mix esr.bootstrap                # default profile
      ESR_PROFILE=staging mix esr.bootstrap

  ## What it runs (in order)

  1. **`mix esr.home.init`** — creates `$ESR_HOME/<profile>/`
     skeleton (credentials, db, snapshots, logs, plugins). Idempotent.
  2. **`mix deps.get`** — ensures dependency tree is present (no-op if
     already fetched).
  3. **`mix esr.home.adopt_db`** — moves any repo-root `esr_core_dev.db`
     into `$ESR_HOME/<profile>/db/esr_core.db`. Idempotent — no-op if
     already migrated.
  ...
  - Mint admin credentials beyond what `esr.home.init` already does
  ...
  @impl Mix.Task
  def run(_argv) do
    Mix.shell().info(banner("Phase 1 — ESR_HOME skeleton (esr.home.init)"))
    Mix.Task.run("esr.home.init", [])

    Mix.shell().info(banner("Phase 2 — Dependencies (mix deps.get)"))
    Mix.Task.run("deps.get", [])

    Mix.shell().info(banner("Phase 3 — DB migration to ESR_HOME (esr.home.adopt_db)"))
    Mix.Task.run("esr.home.adopt_db", [])
  ...
  defp health_check_summary do
    # Start the app so EsrCore.Repo is alive for the SELECT 1 check.
    # Mix is in non-running mode by default; we need just enough to
    # ping the database.
    case Application.ensure_all_started(:esr_core) do
      {:ok, _started} ->
        case Ecto.Adapters.SQL.query(EsrCore.Repo, "SELECT 1", []) do
          {:ok, _} ->
      ...
      {:error, err} ->
        Mix.shell().error("    ✗ Failed to start :esr_core for health check: #{inspect(err)}")
        exit({:shutdown, 1})
    end
  end
end
