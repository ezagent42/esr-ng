defmodule Mix.Tasks.Esr.Plugin.Install do
  ...
  `mix esr.plugin.install` → Kinds + Behaviors reachable without
  restart).

  ## Usage

      mix esr.plugin.install <path>           # path to a built plugin app's _build/<env>/lib/<app>/
      mix esr.plugin.install --app <app_name> # alternative: app already in code path, just start it
  ...
  Two concurrent `mix esr.plugin.install` invocations on the same
  ...
  **Recommendation**: plugin authors should prefer
  ...
  ## Reverse: there is no `mix esr.plugin.uninstall` (intentionally)
  ...
  def run(argv) do
    ...
    case {opts[:app], paths} do
      {nil, []} ->
        Mix.shell().error("usage: mix esr.plugin.install <path-to-plugin> | --app <app_name>")
        exit({:shutdown, 2})
  ...
  defp echo_registrations(app_name) do
    ...
    # use `EsrPlugin<Name>` or similar, so we look for substring
