defmodule Mix.Tasks.Esr.Home.AdoptDb do
  @shortdoc "Move repo-root esr_core_dev.db into $ESR_HOME/<profile>/db/"
  @moduledoc """
  ...
  `$ESR_HOME/<profile>/db/esr_core.db`.

  Concrete moves (only files that exist; idempotent):

      <repo-root>/esr_core_dev.db      → $ESR_HOME/<profile>/db/esr_core.db
      <repo-root>/esr_core_dev.db-shm  → $ESR_HOME/<profile>/db/esr_core.db-shm
      <repo-root>/esr_core_dev.db-wal  → $ESR_HOME/<profile>/db/esr_core.db-wal

  After the move, dev Phoenix server reads the DB path from
  `Esr.Home.path(:db)` via `config/runtime.exs` (no more
  `Path.expand("../esr_core_dev.db", __DIR__)`).

  ## Usage

      mix esr.home.adopt_db
  ...
  - Init runs implicitly if `$ESR_HOME` skeleton is missing.
  ...
  @repo_files [
    {"esr_core_dev.db", "esr_core.db"},
    {"esr_core_dev.db-shm", "esr_core.db-shm"},
    {"esr_core_dev.db-wal", "esr_core.db-wal"}
  ]
  ...
  def run(_argv) do
    ...
    case moved do
      [] ->
        Mix.shell().info("No repo-root dev DB found at #{repo_root}/esr_core_dev.db — nothing to adopt.")
        Mix.shell().info("Dev server already reads from #{target_dir}/esr_core.db on next start.")
