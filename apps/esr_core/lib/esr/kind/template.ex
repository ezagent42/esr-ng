defmodule Esr.Kind.Template do
  ...
  (`"class"` field in the template data); `Esr.TemplateRegistry` maps
  name → module; `Esr.Workspace.Loader` dispatches `instantiate/3` at
  app start to bring the declared Kinds to life.

  This is the **runtime DI form** for plugin authors to add new
  spawnable shapes without touching esr_core — parallel to
  `Esr.SpawnRegistry` (one-Kind-per-URI-scheme) but for whole
  ...
  - `validate/1` — pure shape check called by
    `Esr.Workspace.add_template/3` BEFORE persisting. Fail-fast.
    Optional callback — default `:ok`.
  - `instantiate/3` — effectful. Called by `Esr.Workspace.Loader` at
