defmodule Esr.Kind.Snapshot do
  ...
  - `{:snapshot, :periodic, ms}` — async write every `ms` via
    `Esr.Snapshot.Writer`; restore on boot
  ...
  `Esr.Snapshot.Writer` (mirrors `Esr.Audit.Writer` pattern from
  Decision #60).

  ## Failure mode (Q3)

  Write failure does NOT crash the Kind. Log + `[:esr, :persistence,
  ...
  defp load_with_fallback(uri, kind_module, args) do
    ...
    case fetch_snapshot(uri_str, kind_module) do
      ...
      {:error, reason} ->
        Logger.warning(
          "Esr.Kind.Snapshot: load failed for #{uri_str}: #{inspect(reason)}; using fresh init"
        )
  ...
  def save_now(uri, kind_module, state) do
    ...
    case KindSnapshot.upsert(uri_str, kind_type_str, binary, version) do
      ...
      {:error, reason} = err ->
        Logger.warning(
          "Esr.Kind.Snapshot: save failed for #{uri_str}: #{inspect(reason)}"
        )
