defmodule Esr.RoutingRegistry do
  @moduledoc """
  ...
  Used by `Esr.Routing.Resolver` to derive recipients from a Message
  + current Session context.

  ## How this differs from Esr.BehaviorRegistry and Esr.KindRegistry
  ...
  | `RoutingRegistry` | named table family, plugin-declared | **runtime** (admin runs `mix esr.routing.add_rule`) | **per-table owner pid** (declared at boot) — prevents plugin X from writing to plugin Y's table |
  ...
  ## Table declaration
      ...
      Esr.RoutingRegistry.declare_table(MyPlugin.ChatRouting,
  ...
  ## Write/Read API
      ...
      Esr.RoutingRegistry.put_new(MyPlugin.ChatRouting, key, value)
      Esr.RoutingRegistry.put(MyPlugin.ChatRouting, key, value)

      # read (any process):
      Esr.RoutingRegistry.lookup(MyPlugin.ChatRouting, key)
      Esr.RoutingRegistry.lookup_all(MyPlugin.ChatRouting, key)
      Esr.RoutingRegistry.list_all(MyPlugin.ChatRouting)
      Esr.RoutingRegistry.reverse_index(MyPlugin.ChatRouting, value)

  ## ETS layout

  - `@meta_table` (`:esr_routing_registry_meta`): table_name → %{owner_pid, opts, reverse_index_table?}
  ...
  @meta_table :esr_routing_registry_meta
  ...
  `Esr.Routing.Resolver` iteration. O(n).
  """
