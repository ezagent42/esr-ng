defmodule Esr.Capability.Parser do
  @moduledoc """
  String → `[Esr.Capability.t()]` parser for the `mix esr.user.create`
  ...
  admin runs `mix esr.user.create`). `now` defaults to current UTC.
  """
  @spec parse(String.t(), URI.t(), DateTime.t()) ::
          {:ok, [Esr.Capability.t()]} | {:error, term()}
  ...
  defp resolve_behavior(name) when is_binary(name) do
    ...
    # Convert to module via convention: "chat" → Esr.Behavior.Chat
    capitalized =
      name
      |> String.split("_")
    ...
    module_str = "Elixir.Esr.Behavior." <> capitalized
