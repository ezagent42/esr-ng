defmodule Esr.Runtime do
  ...
  - Runtime: `esr_runtime@<host>` where `<host>` is `127.0.0.1` for
    single-machine dev. Operator can override with `ESR_RUNTIME_NODE` env.
  - CLI: `esr_cli_<os_pid>@<host>` (short-lived, unique per invocation).

  ## Cookie

  Stored at `$ESR_HOME/<profile>/runtime/cookie` (chmod 600). Generated
  ...
  @runtime_node_prefix "esr_runtime"

  @doc "Absolute path of the runtime cookie file under ESR_HOME."
  def cookie_path do
    Path.join(Esr.Home.profile_dir(), "runtime/cookie")
  end
  ...
  @doc """
  The runtime node name (atom). Overridable via `ESR_RUNTIME_NODE` env.
  Default `esr_runtime@127.0.0.1`.
  """
  @spec runtime_node() :: atom()
  def runtime_node do
    case System.get_env("ESR_RUNTIME_NODE") do
      nil ->
  ...
  Called from phx.server boot via `EsrCore.Application.start/2` so the
  ...
  def configure_for_runtime! do
    ...
    case :net_kernel.start([node_name, :longnames]) do
      ...
      err ->
        require Logger
        Logger.warning("Esr.Runtime: net_kernel start failed (#{inspect(err)}); CLI will fall back to error path")
        :ok
    end
  end
  ...
  def connect_as_cli do
    ...
    cli_node = :"esr_cli_#{System.system_time(:nanosecond)}@127.0.0.1"
