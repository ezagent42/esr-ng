defmodule Esr.KindRegistry do
  @moduledoc """
  KindRegistry — URI → pid index for Kind instances.

  Thin wrapper over stdlib `Registry` (started in
  `EsrCore.Application` children as `{Registry, keys: :unique, name:
  Esr.KindRegistry}`). Borrowed pattern from the old esr
  `Esr.Entity.Registry` shape (SPEC §当前 esr 状态对照 borrow #1) but
  ...
  `Esr.RoutingRegistry` and are out of scope for Phase 1.
  ...
  step 2 calls this from `Esr.Kind.Server.init/1`.
  """
