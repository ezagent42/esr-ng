defmodule Esr.Kind do
  @moduledoc """
  Kind — contract for a Kind type module.

  Per DECISIONS P1-D2 / Decision #84: Phase 1 uses the
  `@behaviour Esr.Kind` + shared `Esr.Kind.Server` approach (path B
  in ARCHITECTURE.md §5.7.4), **not** a `use Esr.Kind` macro. The
  Kind module is a pure data declaration: which Behaviors it has,
  what its persistence policy is, etc. The `Esr.Kind.Server`
  ...
  - `behaviors/0`: list of `Esr.Behavior` modules this Kind composes.
