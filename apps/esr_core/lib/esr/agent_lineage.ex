defmodule Esr.AgentLineage do
  ...
  real lineage-aware matching: `Esr.Entity.Agent.spawn/4` records
  ...
  `:esr_agent_lineage` set table owned by `EsrCore.EtsOwner`. Keys
  ...
  Lives in esr_core so it's available before any plugin starts.
  EsrCore.EtsOwner creates the table at boot before plugin
  Application.start callbacks run.
  """

  @table :esr_agent_lineage
