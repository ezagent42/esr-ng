defmodule EsrPluginFeishu.UserBindingTest do
  @moduledoc """
  ...
  test "bind then resolve" do
    ...
    {:ok, _row} = UserBinding.bind(open_id, "user://alice", "user://admin")

    assert {:ok, %URI{} = uri} = UserBinding.resolve(open_id)
    assert URI.to_string(uri) == "user://alice"
  end
  ...
  test "rebind replaces user_uri silently" do
    ...
    {:ok, _} = UserBinding.bind(open_id, "user://alice", "user://admin")
    {:ok, _} = UserBinding.bind(open_id, "user://bob", "user://admin")

    {:ok, uri} = UserBinding.resolve(open_id)
    assert URI.to_string(uri) == "user://bob"
  end
  ...
  test "open_ids_for finds all bindings for a user" do
    user = "user://multi_#{System.unique_integer([:positive])}"
    {:ok, _} = UserBinding.bind("ou_one_#{System.unique_integer([:positive])}", user, "user://admin")
    {:ok, _} = UserBinding.bind("ou_two_#{System.unique_integer([:positive])}", user, "user://admin")
