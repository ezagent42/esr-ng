defmodule EsrPluginFeishu.WsClient do
  @moduledoc """
  ...
  1. Read credentials from `Esr.Home.read_credentials("feishu")`
  ...
  4. For each event: hand off to `EsrPluginFeishu.InboundDispatcher`
  ...
  Set `ESR_FEISHU_WS=0` to skip the WsClient. The plugin still boots
  ...
  alias EsrPluginFeishu.InboundDispatcher
  ...
  def init(_) do
    state = %__MODULE__{
      ...
      enabled?: System.get_env("ESR_FEISHU_WS") != "0"
    }

    cond do
      not state.enabled? ->
        Logger.info("EsrPluginFeishu.WsClient: ESR_FEISHU_WS=0 — staying idle")
        {:ok, state}

      is_nil(state.node_bin) ->
        Logger.warning("EsrPluginFeishu.WsClient: node not found in PATH — WS disabled")
        {:ok, %{state | enabled?: false}}

      not File.exists?(state.sidecar_path) ->
        Logger.warning("EsrPluginFeishu.WsClient: sidecar missing at #{state.sidecar_path}")
        {:ok, %{state | enabled?: false}}
  ...
  def handle_info(:open_sidecar, state) do
    case load_credentials() do
      {:ok, app_id, app_secret, domain} ->
        ...
        Logger.info("EsrPluginFeishu.WsClient: sidecar started (port=#{inspect(port)})")
        {:noreply, %{state | port: port, app_id: app_id, app_secret: app_secret, domain: domain}}

      {:error, reason} ->
        Logger.warning(
          "EsrPluginFeishu.WsClient: cannot start (#{inspect(reason)}); retry in #{@restart_backoff_ms}ms"
        )
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    Logger.warning(
      "EsrPluginFeishu.WsClient: sidecar exited status=#{status}; restart in #{@restart_backoff_ms}ms"
    )
  ...
  defp sidecar_path do
    :code.priv_dir(:esr_plugin_feishu)
  ...
  defp handle_json_line(line) do
    case Jason.decode(line) do
      ...
      {:ok, %{"type" => "connected"}} ->
        Logger.info("EsrPluginFeishu.WsClient: WSS connected")

      {:ok, %{"type" => "disconnected"} = m} ->
        Logger.info("EsrPluginFeishu.WsClient: WSS disconnected: #{inspect(m["reason"])}")

      {:ok, %{"type" => "error", "message" => msg}} ->
        Logger.warning("EsrPluginFeishu.WsClient sidecar error: #{msg}")

      {:ok, _other} ->
        :ok

      {:error, _} ->
        Logger.debug("EsrPluginFeishu.WsClient: non-JSON line from sidecar: #{inspect(line)}")
    end
  end
