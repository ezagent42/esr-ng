defmodule Mix.Tasks.Esr.Feishu.Bind do
  @shortdoc "Bind a Feishu open_id to a local ESR user URI"
  @moduledoc """
  Phase 6 PR 15 — admin CLI for Feishu identity bindings.

      mix esr.feishu.bind ou_6b11faf8e9... user://linyilun
      mix esr.feishu.bind ou_xxx user://linyilun --admin user://admin

  After binding, the bound user receives an `Esr.Capability`
  authorizing dispatch into any `feishu_chat://` Kind (text / image
  / file / future-card / etc.) — see `EsrPluginFeishu.BindingPolicy`.
  ...
  def run(argv) do
    ...
    {open_id, user_uri} =
      case positional do
        [oid, uri] -> {oid, uri}
        _ -> Mix.raise("usage: mix esr.feishu.bind <open_id> <user_uri> [--admin <admin_uri>]")
      end
