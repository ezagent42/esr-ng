defmodule EsrPluginCcChannel.McpConfigWriterTest do
  use ExUnit.Case, async: false

  alias EsrPluginCcChannel.McpConfigWriter
  alias EsrPluginCcChannel.TokenStore

  setup do
    # Isolated $ESR_HOME so TokenStore writes a real but throwaway
    ...
    prev_home = System.get_env("ESR_HOME")
    prev_profile = System.get_env("ESR_PROFILE")
    System.put_env("ESR_HOME", tmp)
    System.put_env("ESR_PROFILE", profile)

    out_dir = Path.join(tmp, "mcp_out")
    File.mkdir_p!(out_dir)

    on_exit(fn ->
      if prev_home, do: System.put_env("ESR_HOME", prev_home), else: System.delete_env("ESR_HOME")

      if prev_profile,
        do: System.put_env("ESR_PROFILE", prev_profile),
        else: System.delete_env("ESR_PROFILE")
  ...
  test "write!/1 emits mcp.json with token + agent_uri in env", %{out_dir: out_dir} do
    ...
    assert env["ESR_AGENT_URI"] == agent_uri
    assert env["ESRD_WS_URL"] == "ws://127.0.0.1:4000/cc_socket/websocket"
    assert is_binary(env["ESR_AGENT_TOKEN"])
    assert String.starts_with?(env["ESR_AGENT_TOKEN"], "tok_")
  end
  ...
  test "write!/1 is token-idempotent — re-write returns the same token", %{out_dir: out_dir} do
    ...
    token1 = path1 |> File.read!() |> Jason.decode!() |> get_in(["mcpServers", "esr-bridge", "env", "ESR_AGENT_TOKEN"])
    ...
    token2 = path2 |> File.read!() |> Jason.decode!() |> get_in(["mcpServers", "esr-bridge", "env", "ESR_AGENT_TOKEN"])
  ...
  test "resolve_ws_url/0 honors env override" do
    prev = System.get_env("ESR_BRIDGE_WS_URL")
    System.put_env("ESR_BRIDGE_WS_URL", "ws://elsewhere:9999/socket")

    try do
      assert McpConfigWriter.resolve_ws_url() == "ws://elsewhere:9999/socket"
    after
      if prev, do: System.put_env("ESR_BRIDGE_WS_URL", prev), else: System.delete_env("ESR_BRIDGE_WS_URL")
    end
  end
