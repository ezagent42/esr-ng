defmodule EsrPluginCcChannel.McpConfigWriter do
  @moduledoc """
  Writes the `.mcp.json` Claude Code consumes via `--mcp-config` for
  the v2 CC channel bridge.

  Replaces `Esr.Bridge.V1Prototype.McpConfigWriter` (HTTP/SSE wire)
  ...
  - **Primary:** `~/.openclaw/esr-ng/bridge.mcp.json` (configurable
    via `Application.get_env(:esr_plugin_cc_channel, :mcp_config_dir)`).
  ...
  `EsrPluginCcChannel.TokenStore`. `write!/1` mints (idempotent) a
  ...
  PtyServer (`apps/esr_plugin_cc_pty/lib/esr/plugin_cc_pty/pty_server.ex`)
  ...
  - `:ws_url` — override the WebSocket endpoint URL (defaults to
    `ESR_BRIDGE_WS_URL` env / `:ws_url` app config /
    `ws://127.0.0.1:4000/cc_socket/websocket`).
  """
  ...
  def write!(opts) do
    agent_uri_str =
      Keyword.get(opts, :agent_uri) ||
        raise ArgumentError,
              "EsrPluginCcChannel.McpConfigWriter.write!/1 requires :agent_uri"

    {:ok, token} = mint_token!(agent_uri_str)

    dir = Keyword.get(opts, :dir, Application.get_env(:esr_plugin_cc_channel, :mcp_config_dir, @default_dir))
    File.mkdir_p!(dir)
    ...
    env = %{
      "ESRD_WS_URL" => ws_url,
      "ESR_AGENT_URI" => agent_uri_str,
      "ESR_AGENT_TOKEN" => token
    }
  ...
  def bridge_script_path do
    Path.expand("../../../python/esr_mcp_bridge.py", __DIR__)
  end
  ...
  Lookup order:
  1. `ESR_BRIDGE_WS_URL` environment variable
  2. `Application.get_env(:esr_plugin_cc_channel, :ws_url)`
  3. `ws://127.0.0.1:4000/cc_socket/websocket` (matches Endpoint mount)
  """
  @spec resolve_ws_url() :: String.t()
  def resolve_ws_url do
    System.get_env("ESR_BRIDGE_WS_URL") ||
      Application.get_env(:esr_plugin_cc_channel, :ws_url) ||
      "ws://127.0.0.1:4000/cc_socket/websocket"
  end
