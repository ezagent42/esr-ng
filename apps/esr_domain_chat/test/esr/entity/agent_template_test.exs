defmodule Esr.Entity.AgentTemplateTest do
  @moduledoc """
  ...
  spawn + slice population is covered by Phase 7 PR 40 (`Esr.Entity.Agent.spawn/4`
  spawn-from-template flow) which exercises the spawn path.
  """

  use ExUnit.Case, async: true

  alias Esr.Entity.AgentTemplate
  ...
  test "behaviors/0 includes Identity (caps + grant policy live on slice)" do
    ...
    assert Esr.Behavior.Identity in behaviors,
  ...
  test "Esr.Kind behaviour callbacks all implemented" do
    ...
    assert callbacks_ok,
           "AgentTemplate must implement all three @impl Esr.Kind callbacks: " <>
