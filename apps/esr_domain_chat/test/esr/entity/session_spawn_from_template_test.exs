defmodule Esr.Entity.SessionSpawnFromTemplateTest do
  @moduledoc """
  Phase 7 PR 41 — Generator (`Esr.Entity.Session.spawn_from_template/2`)
  structural test.
  ...
  test "spawn_from_template/2 exists with documented signature" do
    assert function_exported?(Session, :spawn_from_template, 2),
           "Phase 7 PR 41 — Esr.Entity.Session.spawn_from_template/2 must exist"
  end
  ...
  test "Generator module is Session itself (not a separate Generator module)" do
    # Per SPEC §Generator: "Generator is the role; `Esr.Entity.Session.spawn_from_template/2`
    # is the entry point". Pin that decision — the Generator is not a
    # separate module to avoid plumbing yet another concept.
    refute Code.ensure_loaded?(Esr.Entity.Generator),
           "Generator should NOT be a separate module — it's a role implemented by " <>
             "Esr.Entity.Session.spawn_from_template/2 per SPEC §Generator"
  end
end
