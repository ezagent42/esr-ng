defmodule EsrDomainChat.Integration.RoutingConsolidationInvariantTest do
  @moduledoc """
  ...
  **This test catches future leaks**: if anyone re-introduces a hardcoded
  ...
  > `Esr.Routing.Resolver.resolve/3`. No exceptions.**
  ...
  defp build_msg(text \\ "hi", mentions \\ [], sender \\ "user://admin") do
    %Esr.Message{
  ...
  describe "Resolver is the SINGLE source of routing decisions" do
    test "no rules + no members → no recipients (the gate)" do
      ...
      original_tables =
        Application.get_env(:esr_core, :routing_tables, [
      ...
      Esr.RoutingRegistry.declare_table(empty_table, key_uniqueness: :duplicate)
      Application.put_env(:esr_core, :routing_tables, [empty_table])

      try do
        recipients = Resolver.resolve(build_msg(), URI.parse("session://test"), [])

        assert recipients == [],
               "no rules + no members must produce zero recipients — " <>
                 "if non-empty, a hidden fan-out is leaking in Resolver or upstream caller"
      after
        Application.put_env(:esr_core, :routing_tables, original_tables)
      end
    end

    test "$session_members expands to passed members (excluding sender)" do
      table = :"members_test_#{System.unique_integer([:positive])}"
      Esr.RoutingRegistry.declare_table(table, key_uniqueness: :duplicate)

      original_tables =
        Application.get_env(:esr_core, :routing_tables, [MentionRouting])
      ...
      try do
        ...
        members = [
          URI.parse("user://admin"),
          URI.parse("agent://x"),
          URI.parse("agent://y")
        ]

        msg = build_msg("hi", [], "user://admin")
        recipients = Resolver.resolve(msg, URI.parse("session://test"), members)

        # admin is sender → excluded; remaining 2 → recipients
        assert length(recipients) == 2

        recipient_strs = Enum.map(recipients, &URI.to_string/1) |> Enum.sort()
        assert recipient_strs == ["agent://x", "agent://y"]
      after
        Application.put_env(:esr_core, :routing_tables, original_tables)
      end
    end
    ...
    test "current session is excluded from cross-session targets (recursion guard)" do
      ...
      try do
        current = URI.parse("session://main")

        # Rule routes to current session — Resolver MUST exclude it
        Esr.RoutingRegistry.put(table, Matcher.always(), [URI.to_string(current)])

        recipients = Resolver.resolve(build_msg(), current, [])

        refute Enum.any?(recipients, fn r -> URI.to_string(r) == "session://main" end),
