defmodule EsrDomainChat.Integration.WorkspaceIsolationTest do
  @moduledoc """
  ...
  alias Esr.{Invocation, Message, RoutingRegistry}
  ...
  defp setup_scenario do
    ...
    Application.put_env(:esr_core, :routing_tables, [table_name])
    ...
    {:ok, _} = Esr.SpawnRegistry.spawn(session_a)
    {:ok, _} = Esr.SpawnRegistry.spawn(session_b)

    :ok = Esr.WorkspaceRegistry.bind(session_a, workspace_a)
    :ok = Esr.WorkspaceRegistry.bind(session_b, workspace_b)

    sender = URI.new!("user://#{unique("sender")}")
    eavesdropper = URI.new!("user://#{unique("eavesdropper")}")

    {:ok, _} = Esr.SpawnRegistry.spawn(sender)
    {:ok, _} = Esr.SpawnRegistry.spawn(eavesdropper)
  ...
  defp dispatch_send(session_uri, sender, text) do
    ...
    # Audit writes are async (Esr.Audit.Writer flushes ~every 1s).
    # Force a flush so this test doesn't race the writer.
    if Process.whereis(Esr.Audit.Writer), do: send(Esr.Audit.Writer, :flush)
    Process.sleep(250)
    msg
  end
  ...
  test "unbound session (no WorkspaceRegistry entry) falls back to global semantics" do
    ...
    {:ok, _} = Esr.SpawnRegistry.spawn(session_unbound)
