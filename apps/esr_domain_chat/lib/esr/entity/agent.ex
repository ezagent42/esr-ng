defmodule Esr.Entity.Agent do
  ...
  when a bridge announces itself:
      ...
      Bridge announce (HTTP POST /announce)
        ...
        → DynamicSupervisor.start_child(EsrDomainChat.AgentSupervisor,
  ...
  `Esr.SpawnRegistry.spawn/1` (URI-only per Decision #65), then
  records lineage in `Esr.WorkspaceRegistry` for workspace scope +
  `Esr.AgentLineage` for `{:spawned_by, _}` cap resolution (PR 42
  / Decision #137).
  ...
  - `workspace_uri` — `%URI{}` scope this Agent belongs to;
    bound via `Esr.WorkspaceRegistry.bind/2` so workspace-scoped
  ...
  - `granted_by` — `%URI{}` of the principal authorizing the spawn
    ...
    `Esr.AgentLineage` to support `{:spawned_by, granted_by}`
  ...
  def spawn(%URI{} = _template_uri, instance_name, %URI{} = workspace_uri, %URI{} = granted_by)
    ...
    with {:ok, _pid} <- spawn_or_resume(agent_uri),
         :ok <- Esr.WorkspaceRegistry.bind(agent_uri, workspace_uri),
  ...
  defp record_lineage(agent_uri, granted_by) do
    if Code.ensure_loaded?(Esr.AgentLineage) and function_exported?(Esr.AgentLineage, :record, 2) do
      Esr.AgentLineage.record(agent_uri, granted_by)
    else
      ...
      Logger.debug(
        "Esr.Entity.Agent.spawn: AgentLineage registry not loaded; " <>
