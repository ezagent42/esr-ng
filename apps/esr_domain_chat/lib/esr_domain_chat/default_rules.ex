defmodule EsrDomainChat.DefaultRules do
  @moduledoc """
  ...
  defp ensure_session_members_default_rule do
    if RuleStore.has_system_default?(MentionRouting) do
      ...
      Logger.info(
        "EsrDomainChat.DefaultRules: seeding system_default rule " <>
      ...
      case RuleStore.add(
        ...
        {:error, reason} ->
          Logger.error(
            "EsrDomainChat.DefaultRules: failed to seed system_default rule: #{inspect(reason)}"
          )
