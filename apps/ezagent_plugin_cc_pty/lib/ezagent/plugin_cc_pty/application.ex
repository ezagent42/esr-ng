defmodule EzagentPluginCcPty.Application do
  @moduledoc """
  ...
  1. Register Template Class `Ezagent.PluginCcPty.Template` with
     `Ezagent.TemplateRegistry` so Workspaces can declare cc-pty agents
  ...
  2. Start `EzagentPluginCcPty.PtyServerSupervisor` (DynamicSupervisor) —
  ...
  plugin-isolation north star end-to-end (no ezagent_core / chat plugin
  ...
  def start(_type, _args) do
    children = [
      {DynamicSupervisor, name: EzagentPluginCcPty.PtyServerSupervisor, strategy: :one_for_one}
    ]
    ...
    case Supervisor.start_link(children, strategy: :one_for_one, name: __MODULE__) do
      {:ok, sup_pid} ->
        ...
        # Ezagent.Workspace.Loader.load_all/0 BEFORE this plugin starts —
  ...
  defp register_template_class do
    :ok = Ezagent.TemplateRegistry.register(Ezagent.PluginCcPty.Template)
    :ok
  end
