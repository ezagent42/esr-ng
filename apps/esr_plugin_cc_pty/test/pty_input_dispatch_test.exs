defmodule Esr.PluginCcPty.PtyInputDispatchTest do
  @moduledoc """
  Phase 5 PR 4 invariant — PTY input goes through Esr.Invocation.dispatch
  ...
  test "100-byte stream via dispatch hits PtyServer + bumps slice counters", %{
    ...
    # rather than ==).
    {:ok, kind_pid} = Esr.KindRegistry.lookup(Esr.Entity.PtyInput.default_uri())
  ...
  test "PtyInput singleton alive at boot at pty-input://default" do
    assert {:ok, _pid} = Esr.KindRegistry.lookup(Esr.Entity.PtyInput.default_uri())
  end
  ...
  test "PubSub output topic broadcasts on chunk arrival", %{agent_uri: agent_uri, pty_pid: pid} do
    Phoenix.PubSub.subscribe(EsrCore.PubSub, Esr.PluginCcPty.PtyServer.output_topic(agent_uri))
