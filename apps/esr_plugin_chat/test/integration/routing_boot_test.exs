defmodule EsrPluginChat.Integration.RoutingBootTest do
  @moduledoc """
  ...
  test "MentionRouting table is declared at chat plugin boot" do
    ...
    assert is_list(RoutingRegistry.list_all(EsrPluginChat.Routing.MentionRouting))
  end

  test "SessionRouting table is declared at chat plugin boot" do
    assert is_list(RoutingRegistry.list_all(EsrPluginChat.Routing.SessionRouting))
  end

  test "DefaultRules.bootstrap is idempotent" do
    assert :ok = EsrPluginChat.DefaultRules.bootstrap()
    assert :ok = EsrPluginChat.DefaultRules.bootstrap()
  end
end
