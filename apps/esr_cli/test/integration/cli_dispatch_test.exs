defmodule EsrCLI.Integration.CLIDispatchTest do
  ...
  describe "Dispatch.run_action — end-to-end via auto-derive" do
    test "workspace list_members on an existing workspace returns the member list" do
      ...
      members = [URI.parse("user://admin"), URI.parse("agent://test-cli")]
      {:ok, _pid} = Esr.Workspace.create(name, %{members: members})
      ...
      assert {:ok, %{members: returned}} =
               Dispatch.run_action(
                 Esr.Entity.Workspace,
                 Esr.Behavior.Workspace,
                 :list_members,
                 parsed
               )
    ...
    test "workspace add_member persists + dispatches" do
      ...
      {:ok, _pid} = Esr.Workspace.create(name)

      parsed = %{
        options: %{workspace: name, member: URI.parse("agent://cli-new-member")},
      ...
      assert Enum.any?(members, fn u ->
               URI.to_string(u) == "agent://cli-new-member"
             end)
    end
  end
  ...
  describe "TreeBuilder.build/1 — auto-derive shape" do
    ...
    test "workspace subcommand includes :create facade op (registered by EsrCore.Application)" do
      # Ensure registration ran by booting esr_core
      Application.ensure_all_started(:esr_core)
      Application.ensure_all_started(:esr_cli)
