defmodule EsrCli.Integration.CliLvCapParityTest do
  @moduledoc """
  ...
  BOTH paths derive caps from `Esr.Identity.list_caps_for/1`. This
  ...
  2. CLI's resolve_caller (private, but exercised via the public
     ...
     converge on the SAME `Esr.Identity.list_caps_for/1` result for
     the same user.
  ...
  - Caps that are present in the DB row but not yet in the in-memory
    ...
    which requires Loader-style hydration (see esr.user.create mix
  ...
  - End-to-end dispatch from each surface (covered by
    ...
    reach the SAME `Esr.Invocation.dispatch` function in the
    same BEAM).
  ...
  test "CLI-resolved caps == LV-resolved caps for the same user (V3.4 parity)", %{uri: user_uri} do
    ...
    cli_caps = Esr.Identity.list_caps_for(cli_user_uri)

    # LV path: session cookie → user URI → list_caps_for (same function)
    lv_caps = Esr.Identity.list_caps_for(user_uri)
  ...
  test "non-admin token does NOT resolve to admin wildcard cap", %{uri: user_uri} do
    ...
    caps = Esr.Identity.list_caps_for(returned_uri)
