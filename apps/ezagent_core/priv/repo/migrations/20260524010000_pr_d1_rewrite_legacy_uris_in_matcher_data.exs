defmodule EzagentCore.Repo.Migrations.PrD1RewriteLegacyUrisInMatcherData do
  ...
  "arg": "entity://agent/default/test_demo-builder"}`). Result: rules created before
  ...
  - `messages.sender` / `messages.mentions` (historical audit data —
    a 2026-05-18 message saying `entity://agent/default/test_demo-builder` SAID that, full
  ...
  # `entity://agent/default/cc_X` → legacy `entity://agent/default/test_X` (the form PR-pre-#131 stored)
  # `entity://agent/default/curl_X` → legacy `entity://agent/default/test_X` (the form an admin would have typed)
