defmodule EzagentWeb.CcBridgeAnnounceController do
  @moduledoc """
  ...
  - Records the bridge as connected via `Ezagent.Bridge.V1Prototype.Server.register/2`
  - If `agent_uri` is supplied (Phase 2 path — either
    `EZAGENT_AGENT_URI` from the Python bridge's mcp.json env field, or
    the operator's shell env): spawns an `Ezagent.Entity.Agent` Kind at
    that URI under `EzagentDomainChat.AgentSupervisor`, binds it to
  ...
  Phase 5's ezagent_plugin_cc_channel will use a Phoenix Channel /
  ...
  alias Ezagent.Bridge.V1Prototype.Server, as: BridgeServer
  ...
  defp start_agent_kind(agent_uri) do
    spec = {Ezagent.Kind.Server, {Agent, %{uri: agent_uri}}}

    case DynamicSupervisor.start_child(EzagentDomainChat.AgentSupervisor, spec) do
      {:ok, pid} ->
        {:ok, pid}
  ...
  defp terminate_agent(agent_uri) do
    case Ezagent.KindRegistry.lookup(agent_uri) do
      {:ok, agent_pid} ->
        _ = DynamicSupervisor.terminate_child(EzagentDomainChat.AgentSupervisor, agent_pid)
        :ok
  ...
  def events_sse(conn, %{"bridge_id" => bridge_id}) do
    :ok = Phoenix.PubSub.subscribe(EzagentCore.PubSub, "esr:bridge_v1:to_claude:#{bridge_id}")
