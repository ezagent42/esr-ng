...
- `sub-step-gate.sh` PreToolUse hook 在 commit/tag 时 fire,跑 `mix format --check + mix test + mix esr.check_invariants`
...
**目标**:RoutingRegistry 模块完成 + EsrCore.EtsOwner 加新表 + 单元测试全绿。

**文件**:
- `apps/esr_core/lib/esr/routing_registry.ex`:`declare_table(name, opts)` / `put_new/3` / `put/3` / `lookup/2` / `lookup_all/2` / `list_all/1` / `reverse_index/3`(可选 `opts: [reverse: true]` 时自动维护反向 ETS)。Plugin owner-only-write 通过 `owner_pid` 字段 + write 时 check caller pid
- `apps/esr_core/lib/esr/ets_owner.ex`:加 `Esr.RoutingRegistry.tables_ets` 表 owner

**单元测试**:
- `apps/esr_core/test/esr/routing_registry_test.exs`:declare → put → lookup 路径;put_new 同 key 撞 reject;owner-only-write deny non-owner
- reverse_index 双向 round-trip

**commit message**:`phase3a step 1: Esr.RoutingRegistry + ETS table family`
...
**文件**:
- `apps/esr_core/lib/esr/routing/matcher.ex`:5 个 leaf 构造函数 + `match?(matcher, %Message{}) :: boolean` + `to_json/1` + `from_json/1`

**单元测试**:
- `apps/esr_core/test/esr/routing/matcher_test.exs`:各 leaf match? 正负 case + JSON round-trip 不丢字段

**commit message**:`phase3a step 2: Esr.Routing.Matcher 5 leaf + JSON serde`
...
**文件**:
- `apps/esr_core/lib/esr/routing/resolver.ex`:`resolve(message :: Message, current_session_uri :: URI) :: [recipient_uri]`(内部 hard-code query MentionRouting + SessionRouting 两表;各 rule match?/2 通过则 union receivers;**返 [] 给 caller 表示 fall-through 到 in-session default**)
- `apps/esr_core/lib/esr/routing/rule_store.ex`:Ecto.Schema for `routing_rules` table + `add(table_name, matcher, receivers, created_by)` / `list(table_name)` / `delete(id)`
- `apps/esr_core/priv/repo/migrations/20260517000000_phase3_routing_rules.exs`:CREATE TABLE routing_rules(6 columns,index on table_name)
- `apps/esr_core/priv/repo/migrations/20260517000100_phase3_message_routings.exs`:CREATE TABLE message_routings(message_uri TEXT, session_uri TEXT, inserted_at TIMESTAMP,**复合 PRIMARY KEY (message_uri, session_uri)**,index on session_uri)
- `apps/esr_core/lib/esr/message_store.ex` 改造:
...
**单元测试**:
- `apps/esr_core/test/esr/routing/resolver_test.exs`:Resolver additive(2 rules 都 match → union receivers)+ no match → []
- `apps/esr_core/test/esr/routing/rule_store_test.exs`:add → list round-trip + JSON serde of matcher_data
- `apps/esr_core/test/esr/message_store_test.exs` 加 case:同一 message URI 写入 2 个 session → messages 1 行 + message_routings 2 行;recent_in_session join 正确

**commit message**:`phase3a step 3: Esr.Routing.Resolver + RuleStore + message_routings (multi-session persist)`

**单元测试**:
- `apps/esr_core/test/esr/routing/resolver_test.exs`:Resolver additive 行为(2 rules 都 match → union receivers)+ no match → []
- `apps/esr_core/test/esr/routing/rule_store_test.exs`:add → list round-trip + JSON serde of matcher_data column

**commit message**:`phase3a step 3: Esr.Routing.Resolver + RuleStore + migration`

### 3a-step 4 · Plugin declare tables + 系统默认 rules

**目标**:EsrPluginChat 声明 2 张 routing table + boot 时 idempotent insert 默认 rules + 引入 mix esr.routing.add_rule task。

**文件**:
- `apps/esr_plugin_chat/lib/esr_plugin_chat/application.ex`:start/2 加 `Esr.RoutingRegistry.declare_table(SessionRouting, key_type: :string, value_type: :string)` + `declare_table(MentionRouting, ...)`
...
- `apps/esr_core/lib/mix/tasks/esr.routing.add_rule.ex` (新)~50 LOC:`mix esr.routing.add_rule MentionRouting mention:agent://X receivers:session://Y` 解析 + 写入 RuleStore

**集成测试**:
- `apps/esr_plugin_chat/test/integration/routing_default_test.exs`:boot 后 routing_rules 表已含默认 rules + 第二次 boot 不重复 insert
- `mix esr.routing.add_rule` smoke test

**commit message**:`phase3a step 4: declare routing tables + default rules + mix esr.routing.add_rule`
...
**文件**:
- `apps/esr_web/lib/esr_web/controllers/cc_bridge_announce_controller.ex`:announce/2 删除 `join_agent_to_default_session/1` 调用;只 spawn Agent + bind to bridge_id 留下
- `apps/esr_web/test/esr_web/controllers/cc_bridge_announce_controller_phase2_test.exs`:**改造**(同 commit)— "announce-with-agent-spawns-and-joins" 测试改为 "announce-with-agent-spawns-floating"(断言 agent in KindRegistry 但**不**在任何 session.members);"reply-forwards-to-admin" 测试加 "after manual chat/join" 前置步骤
...
**文件**:
- `apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex`:
...
**测试**:
- `apps/esr_web_liveview/test/admin_live_test.exs`:create 2 sessions + assert sidebar 显示 2 个;click 切换后右侧 stream 更新
...
**文件**:
- `apps/esr_plugin_cc_bridge_v1_prototype/python/esr_mcp_bridge_v1_prototype.py`:
...
- `apps/esr_web/lib/esr_web/controllers/cc_bridge_announce_controller.ex`:`reply/2` 改 require `session_uris: [string]`;optional `ref: string`;调 `Server.forward_reply_to_agent/4`
- `apps/esr_plugin_cc_bridge_v1_prototype/lib/esr/bridge/v1_prototype/server.ex`:`forward_reply_to_agent/4` 改 send `{:reply_received, session_uris, text, ref}` 而非 `{:reply_received, text}`
...
- `apps/esr_plugin_chat/lib/esr/behavior/chat.ex`:
  ...
  - **同一个 message envelope**:`msg = Esr.Message.new(ctx.self_uri, %{text: text, attachments: []}, ref: URI.new!(ref_str_or_nil_to_uri))` — 构造一次,UUID 唯一,跨 N session dispatch 复用(identity invariant)
  ...
  - 若 ref 非 nil:`MessageStore.by_uri(ref_str)` → 比较 `loaded.session_uri` 与 session_uris;不全 match → emit `[:esr, :chat, :reply_session_mismatch]` telemetry + audit warn(仍按 session_uris 路由)
...
**文件**:
- `apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex`:Audit 表加 `result` column color:red 当 event = `reply_session_mismatch`
...
### 3d-step 1 · Esr.Behavior.Identity + kind_server_spec extra args(per #B1)

**目标**:Identity Behavior 在 esr_core 内 + admin User boot 时把 admin_caps 装进 Identity slice。**关键依赖**:`kind_server_spec` 必须升级支持 extra args(Phase 2 只接 uri)。

**文件**:
- `apps/esr_core/lib/esr/behavior/identity.ex` (新)~80 LOC:`@behaviour Esr.Behavior` + actions [`:list_caps`, `:has_cap?`] + state_slice `:identity` + init_slice 接受 `args[:initial_caps]`(默认 `MapSet.new()`)
- `apps/esr_core/lib/esr/entity/user.ex`:behaviors 改 `[Esr.Behavior.Identity]`(从 `[]`);moduledoc 更新("Phase 3 起 admin_caps 通过 Identity slice 持有,不再函数硬编码 — admin_caps/0 函数仍保留供 boot 初始化用")
- `apps/esr_plugin_chat/lib/esr/entity/agent.ex`:behaviors 改 `[Esr.Behavior.Chat, Esr.Behavior.Identity]`(Agent 也持 cap,默认空 MapSet)
...
- `apps/esr_web/lib/esr_web/controllers/cc_bridge_announce_controller.ex`:
  - `start_agent_kind/1` 改造接受 extra args(可选 `initial_caps`,默认 `MapSet.new()`)— Phase 3 Agent 默认空 caps

**测试**:
- `apps/esr_core/test/esr/behavior/identity_test.exs`:Identity invoke `:list_caps` 返 slice.caps;`:has_cap?` 检查
- 集成:admin User spawn 后 `:sys.get_state` → `state.identity.caps == admin_caps`
- 集成:Agent Kind spawn 后 → `state.identity.caps == MapSet.new()`

**commit message**:`phase3d step 1: Esr.Behavior.Identity + admin_caps in slice via kind_server_spec extra_args`

### 3d-step 2 · Esr.Capability.cap_for_action/3 helper(per #P1-8)

**目标**:dispatch step 5.5 需要的"action → cap_needed" 反查函数。**签名加 target URI 参数,从中提取 instance**。

**文件**:
- `apps/esr_core/lib/esr/capability.ex`:加 `cap_for_action(kind_module, action, target_uri :: URI) :: %{kind: atom, behavior: module, instance: URI}`(从 BehaviorRegistry 查 behavior 模块;instance 由 `Esr.URI.instance(target_uri)` 提取)

**测试**:
- `apps/esr_core/test/esr/capability_test.exs`:`cap_for_action(Session, :send, URI.new!("session://main/behavior/chat/send"))` 返 `%{kind: :session, behavior: Esr.Behavior.Chat, instance: URI.new!("session://main")}`

**commit message**:`phase3d step 2: Esr.Capability.cap_for_action/3 helper (with target_uri for instance)`
...
**文件**:
- `apps/esr_core/lib/esr/kind/runtime.ex`:
  - `authz_stub/4` 函数删除
  - 替换为 `authz_check/5`(加 target 参数):
    ...
    granted → emit [:esr, :authz, :granted] + :ok
    !granted → emit [:esr, :authz, :denied] + {:error, :unauthorized}
    ```
...
- `apps/esr_core/lib/esr/audit.ex`:
  ...
  - 加 telemetry handler `[:esr, :authz, :denied]` 记录 + broadcast(同 stub_grant 模式)
  - **保留** `[:esr, :authz, :stub_grant]` handler 兼容(Phase 3d 后理论不该 emit)
- `apps/esr_core/lib/esr/telemetry.ex`:
  - `@events` list 加 `[:esr, :authz, :granted]` + `[:esr, :authz, :denied]`
  - **保留** `[:esr, :authz, :stub_grant]`(legacy,Phase 3d 后不该触发)
- `apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex`:
  - `authz_label/1` 加:`[:esr, :authz, :granted] → "granted"` + `[:esr, :authz, :denied] → "denied"`
  - `result_label/2`:`:denied` row CSS red(同 `reply_session_mismatch` pattern)

**测试**:
- `apps/esr_core/test/esr/kind/runtime_test.exs` 加 case:
  - cap 充足 → dispatch 返 `{:ok, _}` + `:granted` telemetry emit
  - cap 不足 → dispatch 返 `{:error, :unauthorized}` + `:denied` telemetry emit
- `apps/esr_core/test/esr/audit_test.exs` 加:`:granted` event → audit row.authz == "granted"
...
**文件**:
- `apps/esr_plugin_echo/test/integration/f1_direct_invoke_test.exs`:已使用 `Esr.Entity.User.admin_caps()`,不需改
- `apps/esr_plugin_chat/test/esr/behavior/chat_test.exs`:ctx fixtures 加 `caps: Esr.Entity.User.admin_caps()`
- `apps/esr_plugin_chat/test/integration/chat_routing_test.exs`:同上
- `apps/esr_web/test/esr_web/controllers/cc_bridge_announce_controller_phase2_test.exs`:同上(注 dispatch path)
- `apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex`:`ctx/0` 函数已经使用 admin_caps,但加 helper 确保 dispatch path 一致
- 新 helper:`apps/esr_core/test/support/cap_helpers.ex` (新) — `granting_caps(needed_cap_shape)` 生成精确单一 cap
...
**文件**:
- `apps/esr_core/lib/mix/tasks/esr.check_invariants.ex`:
  - `check_invariant_9/0`(grep `:stub_grant` 带 colon prefix in code,排除 docstring/comment)— allowlist: telemetry @events list 内的 `[:esr, :authz, :stub_grant]`(legacy 兼容保留)
  - `check_invariant_10/0`(grep `Capability.matches?` in `kind/runtime.ex`)— **tripwire only**,真正语义验证靠 runtime test
- moduledoc 更新
- `apps/esr_core/test/esr/kind/runtime_phase3d_test.exs` (新)— **runtime invariant test**:构造一个 ctx 带空 caps + 一个 Echo target → dispatch → 断言 `{:error, :unauthorized}` + audit row.authz == "denied"。**此测试 fail = #10 invariant 真正失效**(per memory `feedback_completion_requires_invariant_test`)

**测试**:
- 运行 `mix esr.check_invariants` exit 0
- 运行新 runtime test 绿
...
2. **USER ACTION REQUIRED**:Allen 终端跑 `bash scripts/cc-bridge-attach.sh`(.local.sh.example 已 ESR_AGENT_URI 配)
...
6. agent-browser:mix esr.routing.add_rule MentionRouting text_contains:urgent receivers:session://review + send "urgent" in main + verify main 和 review 都收到
