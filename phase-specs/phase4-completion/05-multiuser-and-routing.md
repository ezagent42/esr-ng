...
- Both share the Phase 3d real-cap-check substrate (`Esr.Kind.Runtime.authz_check/4` at `apps/esr_core/lib/esr/kind/runtime.ex:110-134`). Part A makes that path *fire* in the real UI; Part B *exercises* it from a second admin surface.
...
```elixir
# apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex:473-479
defp ctx do
  %{
...
1. **The cap check is never exercised in production.** Phase 3d shipped the real `Capability.matches?/2` gate at `runtime.ex:110-134`, but admin's triple-`:any` cap (`Esr.Entity.User.admin_caps/0` at `apps/esr_core/lib/esr/entity/user.ex:46-57`) grants everything. `[:esr, :authz, :denied]` telemetry never fires from the UI. The audit pipeline at `apps/esr_core/lib/esr/audit.ex:33-42` listens for it; the codepath is dead in production.
2. **Identity Behavior is single-tenant.** `Esr.Behavior.Identity` (`apps/esr_core/lib/esr/behavior/identity.ex`) holds per-user caps in `:identity` slice — built for many Users, used by one (admin).
3. **No path to add a User.** No `mix esr.user.create`, no /login, no session. Adding a non-admin User would require code-editing `chat plugin Application.start` (where admin is spawned).
...
### A.2.1 User provisioning (`mix esr.user.create`)

**Where:** new `apps/esr_core/lib/mix/tasks/esr.user.create.ex` (parallel to existing `esr.routing.add_rule.ex` task pattern).
...
- `kind.behavior` → `%Capability{kind: :workspace, behavior: Esr.Behavior.Workspace, instance: :any, ...}`
...
1. **Parse + validate caps grammar.** Lookup behavior module names via `Esr.BehaviorRegistry`; reject unknown atoms early (memory `feedback_let_it_crash_no_workarounds`: fail at user-action time, not at next boot).
...
3. **If the BEAM is running** (interactive `mix esr.user.create` against a live node — Phase 4 nice-to-have, see Q-MU-3): also spawn the User Kind via `Esr.SpawnRegistry.spawn(URI.parse(user_uri))` so the new principal is live immediately, no restart.
...
- The User Kind's `:identity` slice (`apps/esr_core/lib/esr/behavior/identity.ex:23-25`) is `%{caps: MapSet.t(Capability.t())}` — runtime shape, MapSet not JSON-native, and caps are *granted* not *configured*.
...
**New module:** `apps/esr_web/lib/esr_web/controllers/session_controller.ex`
...
**Router additions** (`apps/esr_web/lib/esr_web/router.ex` after line 24):
```
scope "/", EsrWeb do
...
**Plug `:require_user`** (new `apps/esr_web/lib/esr_web/plugs/require_user.ex`):
...
def mount(_params, session, socket) do
  ...
  {:ok, caps} = Esr.Identity.list_caps_for(caller_uri)
  socket = socket
...
- **Admin always exists.** `Esr.Entity.User.admin_uri/0` + `admin_caps/0` are structural per Decision #81; the migration's seed row mirrors them.
- **Admin's password:** seed-time the admin row has a password but no one knows it. Provide `mix esr.user.set_password user://admin --password '...'` (or have it print a generated password at first migration — see Q-MU-1).
- **Adding more users:** `mix esr.user.create` only. No LV UI for user management in Phase 4 — that's a Phase 5 admin surface. **Listing** users in a read-only `/admin/users` page is a 30-LOC stretch goal — recommend deferring to Phase 5 to keep this PR scoped.
...
**Operator (admin):**
1. After migration: `mix esr.user.set_password user://admin --password 'admin-pw'` (one-time).
2. To onboard Allen: `mix esr.user.create user://allen --password 'allen-pw' --caps 'chat.send,workspace.read'`.
...
3. If wanted: `mix esr.user.create allen2 --caps '*' --allow-allcaps` to make a second admin.

## A.4 Dev-author experience (Part A)

**Zero impact on plugin authors.** Multi-user is entirely inside `esr_web` + `esr_core` + chat plugin's Application bootstrap. The plugin-isolation north star (memory `feedback_north_star_plugin_isolation`) holds: a plugin author writes Kinds + Behaviors, never touches login or `users`.

**One contract emerges:** `Esr.Identity.list_caps_for(uri)` becomes the canonical way to get a principal's caps from any non-LV code path (Phase 5 Feishu plugin will use this to derive ctx for a Feishu-originated invocation, instead of inlining `User.admin_caps/0`).

## A.5 Decision points — Multi-user

| # | Question | Default if you don't answer |
|---|----------|------------------------------|
| Q-MU-1 | Admin's initial password: **generate-and-print at migration** (one-time secret, must capture) vs **require `mix esr.user.set_password` before first login** (zero secret in logs). | Require `set_password` first; migration leaves `password_hash` empty and SessionController refuses login for empty-hash rows. |
...
| Q-MU-3 | `mix esr.user.create` on a **live BEAM**: spawn the new User Kind immediately (no restart) vs **require restart** to pick up the new user. Live spawn requires the mix task to connect to the running node or share a code path with the plugin's Application.start. | Live spawn via `SpawnRegistry.spawn/1` if the chat plugin app is started in the mix task's BEAM (it is — `Application.ensure_all_started(:esr_plugin_chat)` like the routing task does at `esr.routing.add_rule.ex:45`). |
| Q-MU-4 | Auth backend: **bcrypt password** (this spec) vs **opaque token** (`mix esr.user.create allen --token X` and user pastes token into a form). Token is simpler crypto but worse UX. | Bcrypt password. `:bcrypt_elixir` is the de-facto Elixir choice. |
...
|------|----------|---------|
| `users` table + `Esr.Users` facade unit | `apps/esr_core/test/esr/users_test.exs` (new) | create / lookup-by-uri / password verify / cap-decode round-trip |
| Caps parser unit | `apps/esr_core/test/esr/capability_parser_test.exs` (new) | `"chat.send"` → correct shape; `"*"` rejected without flag; instance-scoped grammar; bad input fails fast |
| SessionController integration | `apps/esr_web/test/esr_web/controllers/session_controller_test.exs` (new) | GET form / POST happy / POST bad creds / POST unknown user / logout clears session |
| `:require_user` plug | `apps/esr_web/test/esr_web/plugs/require_user_test.exs` (new) | bounces unauthed / passes authed |
| **LV cap-deny integration** ★ | `apps/esr_web_liveview/test/esr_web_liveview/cap_deny_integration_test.exs` (new) | Spawn user with `{chat.send}` only; mount admin_live with that session; submit "Add member" → assert flash contains "permission" + assert `:denied` telemetry fired with `caller=user://allen` |
| Audit row for denied | extend `apps/esr_core/test/esr/audit_test.exs` | Trigger denied via dispatch → assert SQLite row with `authz="denied"` |
...
| File | New/Δ | LOC |
|------|-------|-----|
| `apps/esr_core/lib/mix/tasks/esr.user.create.ex` | New | ~70 |
| `apps/esr_core/lib/mix/tasks/esr.user.set_password.ex` | New | ~40 |
| `apps/esr_core/lib/esr/users.ex` (facade: create/lookup/verify) | New | ~70 |
| `apps/esr_core/lib/esr/identity.ex` (`list_caps_for/1` facade) | New | ~25 |
| `apps/esr_core/lib/esr/capability_parser.ex` (string → cap struct) | New | ~50 |
| `apps/esr_core/priv/repo/migrations/<ts>_create_users.exs` | New | ~25 |
| `apps/esr_core/lib/esr/behavior/identity.ex` (Q-MU-5 self-grant) | Δ | +5 |
| Chat plugin `Application.start` (load users → spawn all) | Δ | +20 |
| `apps/esr_web/lib/esr_web/controllers/session_controller.ex` | New | ~60 |
| `apps/esr_web/lib/esr_web/controllers/session_html.ex` + EEx templates | New | ~40 |
| `apps/esr_web/lib/esr_web/plugs/require_user.ex` | New | ~20 |
| `apps/esr_web/lib/esr_web/router.ex` (login routes + plug) | Δ | +12 |
| `apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex` (ctx + flash polish) | Δ | +25 |
| `apps/esr_web_liveview/lib/esr_web_liveview/workspaces_live.ex` (same ctx shift) | Δ | +10 |
| `apps/esr_web_liveview/lib/esr_web_liveview/workspace_detail_live.ex` (same ctx shift) | Δ | +10 |
| **Subtotal impl** | | **~480** |
...
- `Phoenix.ConnTest` for controller tests (already in `:esr_web` test deps).
...
1. **No combinators.** `Esr.Routing.Matcher` (`apps/esr_core/lib/esr/routing/matcher.ex`) ships 5 leaf matchers (`mention/from/text_contains/text_matches/always`). The moduledoc explicitly defers and/or/not to "Phase 4+". Today admins can express "urgent" OR "@oncall" only as **two separate rules** (additive semantics per Decision #41). Real scenarios — e.g. "urgent text AND from cc-builder" — cannot be expressed.

2. **No LV editor for routing rules.** Today rules are added via `mix esr.routing.add_rule` (`apps/esr_core/lib/mix/tasks/esr.routing.add_rule.ex`) only. `WorkspaceDetailLive` (`apps/esr_web_liveview/lib/esr_web_liveview/workspace_detail_live.ex:160-169`) renders `routing_rules` as a read-only `<pre>` JSON block. Admin can't add/edit/delete rules from a browser.

3. **`Workspace.routing_rules` is dead config.** The Workspace Behavior persists `routing_rules :: [map]` (`apps/esr_core/lib/esr/behavior/workspace.ex:11-12,115-117`) and the Loader passes them through to `init_slice` (`apps/esr_core/lib/esr/workspace/loader.ex:59`), but **nothing reads them**. `Resolver` queries only the plugin-declared global tables (`MentionRouting`, `SessionRouting` — declared at `apps/esr_plugin_chat/lib/esr_plugin_chat/application.ex:155-158`) via `Application.get_env(:esr_core, :routing_tables, @default_routing_tables)` (`resolver.ex:51`). Workspace-scoped rules don't exist as a runtime concept.
...
**Extension to `Esr.Routing.Matcher` type:**
...
**Mix task parser extension** (`esr.routing.add_rule.ex:71-92`):
...
- **(b) JSON literal in flag:** `mix esr.routing.add_rule Tbl --matcher-json '{"type":"and","items":[...]}' receivers:...` — bypass the spec string entirely for combinators.
...
- New `apps/esr_web_liveview/lib/esr_web_liveview/routing_live.ex`.
- Lists rows from `RuleStore.list_global/0` grouped by `table_name`.
- "Add rule" form: table picker (dropdown of `Esr.RoutingRegistry`'s declared tables — read via a new `RoutingRegistry.list_tables/0`), matcher builder (B.2.3.3), receivers (multi-select of known URIs from `KindRegistry.list_all`).
...
- Same form component (`EsrWebLiveview.RuleFormComponent`) imported in both pages.
...
- Two real options:
  - **(a) Define a synthetic Kind** `Esr.Kind.RoutingAdmin` with one Behavior `Esr.Behavior.RoutingAdmin` exposing `:add_rule` / `:delete_rule` actions. Then the cap shape is natural: `routing_admin.routing_admin@*`. Rule-edit goes through dispatch like everything else (cap check fires, audit row written).
  - **(b) Bypass dispatch for rule edits;** add a plain function-cap check `Esr.Routing.RuleStore.add/4` does at its entry, looking at ctx.caps for a specific marker cap. Inconsistent with the rest of the system.
...
- **`Esr.Routing.Matcher` API additions** (`all_of`/`any_of`/`negate`) are pure extensions. Existing plugin code using `Matcher.mention/from/etc.` unchanged.
...
| Test | Location | Asserts |
|------|----------|---------|
| Matcher combinator unit | `apps/esr_core/test/esr/routing/matcher_combinator_test.exs` (new) | and/or/not evaluation; constructors; nested combos; JSON round-trip incl. nested; depth-limit enforcement (Q-RT-1); empty list cases (`{:and, []}` → true vacuously? recommend define explicitly) |
| RuleStore workspace_uri column | extend `apps/esr_core/test/esr/routing/rule_store_test.exs` | add with workspace_uri / list_by_workspace / list_global / migration runs cleanly with existing rows |
| RoutingRegistry.list_tables | extend `apps/esr_core/test/esr/routing_registry_test.exs` | returns all declared tables; respects owner-pid; empty when none declared |
| RoutingAdmin Kind unit | `apps/esr_core/test/esr/kind/routing_admin_test.exs` (new) | `:add_rule` action wraps `RuleStore.add`; cap check fires (Part A test pattern reused) |
| **LV rule add round-trip** ★ | `apps/esr_web_liveview/test/esr_web_liveview/routing_live_test.exs` (new) | Mount `/admin/routing` as admin → submit leaf rule → assert row in RuleStore + appears in live list; submit combinator JSON → same assertion; submit invalid JSON → flash error, no row written |
| **Workspace-scoped rule round-trip** | extend `apps/esr_web_liveview/test/esr_web_liveview/workspace_detail_live_test.exs` | Add rule via Workspace detail form → row has `workspace_uri = ws.uri` → appears in both /admin/routing global list (with scope badge) and Workspace's local list |
...
| File | New/Δ | LOC |
|------|-------|-----|
| `apps/esr_core/lib/esr/routing/matcher.ex` (combinators + JSON + constructors) | Δ | +60 |
| `apps/esr_core/lib/esr/routing/rule_store.ex` (workspace_uri column + list_by_workspace/global) | Δ | +35 |
| `apps/esr_core/priv/repo/migrations/<ts>_routing_rules_workspace_uri.exs` | New | ~15 |
| `apps/esr_core/lib/esr/routing_registry.ex` (`list_tables/0`) | Δ | +10 |
| `apps/esr_core/lib/esr/kind/routing_admin.ex` + Behavior + spawn fn | New | ~80 |
| `apps/esr_core/lib/mix/tasks/esr.routing.add_rule.ex` (`--matcher-json` flag) | Δ | +25 |
| `apps/esr_core/lib/esr/behavior/workspace.ex` (deprecate `:set_routing_rules`) | Δ | +5 |
| `apps/esr_web_liveview/lib/esr_web_liveview/routing_live.ex` | New | ~150 |
| `apps/esr_web_liveview/lib/esr_web_liveview/components/rule_form_component.ex` | New | ~110 |
| `apps/esr_web_liveview/lib/esr_web_liveview/workspace_detail_live.ex` (replace pre with editor) | Δ | +30 |
| `apps/esr_web/lib/esr_web/router.ex` (`/admin/routing` route) | Δ | +1 |
| **Subtotal impl** | | **~520** |
...
| Existing call to `Esr.Workspace.set_routing_rules/2` | Logs deprecation warning, no-op on RuleStore (writes to Workspace slice only — slice-only persistence harmless until column dropped Phase 5). |
...
1. **Login behavior change is the riskiest UX delta.** Today every dev who runs `mix phx.server` lands on `/admin` as full admin. After this PR they need to remember a password. Mitigation: dev-mode shortcut env var `ESR_DEV_AUTOLOGIN=user://admin` that bypasses `:require_user` (only in `:dev` mix env). ~10 LOC. Should this be in scope or out of scope? **Suggest in scope for A-ui PR.**

2. **The `Esr.Identity.list_caps_for/1` self-grant (Q-MU-5 option a) is subtle.** Auto-adding a self-cap to every spawned User means the User's `:identity/list_caps` action is always callable by themselves. Fine for caps reading. But it sets a precedent: "every Kind has implicit cap on itself." Phase 5 might want this for other Behaviors too (e.g. a User can always set their own avatar). **Flag this as a pattern that will recur**, not a one-off.
