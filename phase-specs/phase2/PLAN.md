...
### 2a-step 1 · Esr.Message 数据型 + Ecto custom URI type

**目标**:Message struct + Jason 序列化 + Ecto URI type,单元测试全绿。零 dispatch / Kind 依赖。

**文件**:
- `apps/esr_core/lib/esr/message.ex`:6 字段 defstruct + `new(sender, body, opts \\ [])` + `Jason.Encoder` impl for both `%Esr.Message{}` 和 `%URI{}`
- `apps/esr_core/lib/esr/ecto/uri_type.ex`:custom Ecto type,`load/dump/cast` 实现 URI struct ↔ string round-trip

**单元测试**:
- `apps/esr_core/test/esr/message_test.exs`:new/3 各 opt 组合 / URI auto-gen 唯一性 / Jason encode round-trip
- `apps/esr_core/test/esr/ecto/uri_type_test.exs`:URI struct ↔ string 双向转

**commit message**:`phase2a step 1: Esr.Message struct + Ecto custom URI type`
...
**文件**:
- `apps/esr_core/priv/repo/migrations/<timestamp>_phase2_messages.exs`:CREATE TABLE messages(7 列)+ 2 index(per §10.4)
- 升级 `apps/esr_core/lib/esr/message.ex`:加 `use Ecto.Schema` + schema 块(field types 跟 column types 对齐;URI 字段用 `Esr.Ecto.URI`;body / mentions 用 `:map`,ecto_sqlite3 自动 JSON-encode)

**集成测试**:
- 跑 `mix ecto.migrate`(等价于 reset DB)
- `apps/esr_core/test/esr/message_test.exs` 加 schema 测试:Repo.insert(%Esr.Message{...} as struct) + Repo.get round-trip

**commit message**:`phase2a step 2: SQLite messages migration + Esr.Message as Ecto.Schema`

---

### 2a-step 3 · Esr.MessageStore API + Esr.Behavior.Chat 接口契约

**目标**:MessageStore 4 函数完整;Chat Behavior 模块声明 actions/0 + interface/0(invoke 留 stub for 2b)。

**文件**:
- `apps/esr_core/lib/esr/message_store.ex`:`write/2`, `in_session_since/2`, `recent_in_session/2`, `by_uri/1` — 全 Ecto query DSL
- 新 plugin `apps/esr_plugin_chat/`:
  - `mix.exs`:depends on esr_core,esr_plugin_chat is umbrella app
  - `lib/esr_plugin_chat/application.ex`:Application.start/2(2a 只 declare,无 children)
  - `lib/esr/behavior/chat.ex`:@behaviour Esr.Behavior;actions/0 → `[:send, :receive, :join, :leave]`;interface/0 用 `%Esr.Message{}` schema(`:send` action args)+ stub send schema for `{:join, :leave}` (URI map);invoke/4 全 clauses 返回 `{:error, :not_implemented_in_2a}`

**单元测试**:
- `apps/esr_core/test/esr/message_store_test.exs`:write/in_session_since(asc order)/recent_in_session(desc limit)/by_uri,使用 sandbox checkout
- `apps/esr_plugin_chat/test/esr/behavior/chat_interface_test.exs`:`Esr.Behavior.Chat.actions/0` 返回 4 element + `interface/0` 形状(可被 InterfaceValidator 调用)

**commit message**:`phase2a step 3: Esr.MessageStore + Esr.Behavior.Chat interface contract`
...
**文件**:
- `apps/esr_plugin_chat/lib/esr/entity/session.ex`(~15 LOC):@behaviour Esr.Kind,type_name :session,behaviors `[Esr.Behavior.Chat]`,persistence :ephemeral
...
**Phase 1 `Esr.Entity.User` 升级(此 step 完成)**:删除 Phase 1 的 stub callbacks(behaviors `[]`),改为 `behaviors [Esr.Behavior.Chat]`;保留 admin_uri/0 + admin_caps/0 函数;type_name :user;persistence :ephemeral(Phase 2 同形)

**单元测试**:
- spawn Session Kind via `Esr.Kind.Server.start_link/1`,确认在 KindRegistry 里
...
- `apps/esr_plugin_chat/lib/esr/behavior/chat.ex` 大改:
  ...
  - `invoke(:receive, slice, %Message{} = msg, ctx)`:从 `ctx.kind_module` 分支:Agent → push SSE(经 `Esr.Bridge.V1Prototype.Server` to_claude topic);User → `Phoenix.PubSub.broadcast(EsrCore.PubSub, "esr:user:<self_uri>:events", {:message_received, msg})`(LV 订阅 — 注意 `:events` 后缀,跟 audit / session events 一致,不用 `:inbox` 避免 invariant #1 allowlist 复杂化)
...
- `Esr.Kind.Server` 加 `handle_info({:DOWN, ref, :process, _pid, _reason}, state)`:delegate to `Esr.Behavior.Chat.handle_member_down(slice, ref)` 如果 kind 模块 declare 该 hook;否则 default ignore
...
**`mix esr.check_invariants` 升级**(2b-step 2 同 commit):invariant #1 grep 的 file allowlist 加 `apps/esr_plugin_chat/lib/esr/behavior/chat.ex`(`Chat.invoke(:send, ...)` 和 `:receive` 都用 `PubSub.broadcast` 走 `:events` topic — 跟 audit.ex / invocation.ex 同形)。`Esr.Kind.Runtime` 加 `kind_module` 到 ctx 也是同 step。
...
- `apps/esr_web/lib/esr_web/controllers/cc_bridge_announce_controller.ex`:
  - `announce/2`:除原本 register Server,**加** spawn `Esr.Entity.Agent` Kind via DynamicSupervisor.start_child(at agent_uri)
  - `reply/2`:**删除** Server.record_reply 路径;改为 `Esr.Bridge.V1Prototype.Server.forward_reply_to_agent(bridge_id, text)`
  - `disconnect/2`:除 unregister,**加** terminate Agent Kind
- `apps/esr_plugin_cc_bridge_v1_prototype/lib/esr/bridge/v1_prototype/server.ex`:
...
- `apps/esr_plugin_chat/lib/esr/entity/agent.ex` 加 `handle_info({:reply_received, text}, state)`(via `Esr.Kind.Server` 的 forward-to-behavior 机制):构造 `%Esr.Message{sender: self_uri, body: %{text, attachments: []}}` → dispatch `session://main/behavior/chat/send`(via Esr.Invocation.dispatch)
...
**文件**:`apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex` 大改 render/1:
...
mount/3 改动:
- 加载 `Esr.MessageStore.recent_in_session(URI.parse("session://main"), 50)` 进 stream
...
`Esr.Kind.Server.handle_info({:DOWN, ...})` 怎么把 message 转给 Behavior?Phase 1 Kind.Server 没暴露 Behavior-level handle_info hook。两个原则:
...
- (a) ESR 接受两个 Agent Kind spawn(同 agent_uri → put_new conflict crash 第二个;不同 agent_uri → 都活)
- (b) ESR reject 第二个 announce HTTP
...
```
/goal Work in /Users/h2oslabs/Workspace/esr-ng/.claude/worktrees/phase-2/ — cd there + verify git branch is phase-2.

Implement esr-ng Phase 2 sub-steps 2a → 2b → 2c. Tags phase2a, phase2b, phase2 pushed.
...
3. sub-step-gate.sh green at all 3 tag commits (mix format / mix test / mix esr.check_invariants)
...
- ZERO hardcoded URI in code (P-4: ESR_AGENT_URI env-driven)
...
- mix esr.check_invariants 6/6 still clean
