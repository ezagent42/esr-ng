...
把 esr_core 从 "能 invoke 单个 behavior 的 router" 升级到 "能在多个 agent 之间 route Message 的 router"。这是 Decision #61("ESR 是 router 不是 req/resp app")在代码层的**第一次真正落地** — 让 LV(代表 Allen)和 claude(经 bridge 接入)在 ESR 内部**对称地**作为 Session 内 chat 参与者,通过 Message envelope 互发。

Phase 1 → Phase 2 关键不变量变化:LV 输入和 claude 回复**在系统里成为同种数据**(`%Esr.Message{}`),走同一条 dispatch 路径(`session://main/behavior/chat/send`),持久化到同一表(`messages`),LV 上**视觉上 indistinguishable**(无 Phase 1 的"← from claude"独立面板)。

## 测试员体验 / demo

完成 Phase 2 后 Allen 能做到:

1. 启动 `mix phx.server`(ESR)+ 另一个终端 `bash scripts/cc-bridge-attach.sh`(真 claude session)
...
|---|---|---|---|---|
| **2a** | data 层 | `Esr.Message` + `Esr.MessageStore`(schemaful Ecto) + `Esr.Behavior.Chat` 接口契约 + SQLite migration | ~150 core(message ~25 + message_store ~50 + chat 接口 ~30 + migration ~20 + tests ~25 LOC reserved) | `phase2a` |
| **2b** | router 接线 | `Esr.Entity.Session` / `User`(升级真 Kind)/ `Agent` + Chat Behavior 4 actions invoke + Process.monitor + boot/dynamic spawn + LV 加 Session 成员显示 | ~250 LOC across plugins + 改 esr_core Application children | `phase2b` |
...
构造器:`Esr.Message.new(sender, body, opts \\ [])`(idiomatic Elixir,sender+body 必填,其余 keyword opts)。identity invariant 仍锁原 5 字段(uri 是 identity reference 不是 identity payload)。

### P-2 Chat Behavior

- Module:`Esr.Behavior.Chat`
- Actions menu:`[:send, :receive, :join, :leave]`
- Per-Kind register subset:
  - `Esr.Entity.Session` 注册 `{Session, :send}` + `{Session, :join}` + `{Session, :leave}`
  - `Esr.Entity.Agent` 注册 `{Agent, :receive}`
  - `Esr.Entity.User` 注册 `{User, :receive}`

### P-3 K 路径 dispatch + state shape

K 路径(每个外部参与方有自己的 Kind 持 Chat Behavior 的 receive 端)— ESR-as-router 真落地。Session.Chat state slice:
...
- Python bridge 启动通过 env `ESR_AGENT_URI` 拿 preferred URI(`cc-bridge-attach.local.sh` 配,gitignored,operator 自定义)
- bridge announce body 多带 `agent_uri` 字段
- ESR `CcBridgeAnnounceController.announce` 动态 spawn `Esr.Entity.Agent` Kind 实例
...
- bridge POST `/api/cc-bridge/reply` 时:
  - Controller 收到 → 调 `Esr.Bridge.V1Prototype.Server.forward_reply_to_agent(bridge_id, text)`
  - Server 通过 bridge_id 找对应 Agent Kind pid → 发 `:reply_received` message
  - Agent Kind 自己 GenServer 内构造 `%Esr.Message{sender: self_uri, body, mentions: ...}` → dispatch `session://main/behavior/chat/send`
  - 这跟 LV 提交时路径**完全对称**(LV 也是构造 Message → dispatch `session://main/behavior/chat/send`)

## 2a Deliverables

**核心 4 模块**:
- `apps/esr_core/lib/esr/message.ex`(~25 LOC):defstruct 6 字段 + new/3 + `Jason.Encoder` impl(URI 字段 stringify)+ optional `Ecto.Schema` 同 file
- `apps/esr_core/lib/esr/message_store.ex`(~50 LOC):4 函数 API(`write/2`, `in_session_since/2`, `recent_in_session/2`, `by_uri/1`),Ecto query DSL
- `apps/esr_core/lib/esr/ecto/uri_type.ex`(~15 LOC):Ecto custom type `Esr.Ecto.URI`,implement `load/1` `dump/1` `cast/1`,allow URI struct ↔ string 自动转
- `apps/esr_plugin_chat/lib/esr/behavior/chat.ex`(~30 LOC,plugin 形式):`@behaviour Esr.Behavior`,actions/0 返回 4-element menu,interface schema 声明 `:send` args 是 `%Esr.Message{}`;invoke 各 clause 暂留 `{:error, :not_yet_implemented_in_2a}`(2b 才写完)

**Ecto migration**(`apps/esr_core/priv/repo/migrations/...phase2_messages.exs`):
- `messages` 表 7 列:`uri PK / session_uri / sender / mentions JSON / body JSON / ref / inserted_at`
- 2 索引:`(session_uri, inserted_at)` + `(sender)`

**测试**(`apps/esr_core/test/esr/`):
...
- `session.ex`(~15 LOC):@behaviour Esr.Kind,type_name :session, behaviors [Esr.Behavior.Chat], persistence :ephemeral(state 不持久,只在 GenServer 内)
- `agent.ex`(~15 LOC):type_name :agent, behaviors [Esr.Behavior.Chat], persistence :ephemeral
- **Phase 1 `Esr.Entity.User` 升级**:从 stub Kind 改为真 Kind — behaviors `[Esr.Behavior.Chat]`,admin_uri/admin_caps 保留

**Chat Behavior 4 invoke clauses 完整实现**(`apps/esr_plugin_chat/lib/esr/behavior/chat.ex` 接入主体):
- `invoke(:send, slice, %Esr.Message{} = msg, ctx)`:写 MessageStore + broadcast session events PubSub + for each mention in online: dispatch URI/behavior/chat/receive
- `invoke(:receive, slice, %Esr.Message{} = msg, ctx)`:通过 **`ctx.kind_module`**(由 `Esr.Kind.Runtime` 在调 invoke 前注入)分支:
  - `ctx.kind_module == Esr.Entity.Agent` → push to bridge SSE(经 `Esr.Bridge.V1Prototype.Server` 的 per-bridge to_claude topic)
  - `ctx.kind_module == Esr.Entity.User` → `Phoenix.PubSub.broadcast(EsrCore.PubSub, "esr:user:<self_uri>:events", {:message_received, msg})`(LV /admin 订阅)
  - `kind_module` ctx injection 是 2b-step 1 对 `Esr.Kind.Runtime.handle_dispatch/3` 的 1-line patch
...
**Esr.Kind.Server `handle_info` 扩展**(2b-step 2 + 2c-step 1 一起做):

`Esr.Kind.Server.handle_info/2` 现在只 match 几个 Phase 1 case;Phase 2 加**unmatched message forwarder**:
...
这两个 hook 用同一种"defensive forward unmatched info messages to Behavior modules"机制,实现一次。`Esr.Behavior.Chat` 实现两个 hook。`Esr.Entity.Agent` 通过 Chat Behavior 的 `handle_kind_message/3` 接到 `:reply_received`,内部构造 `%Esr.Message{}` + dispatch `session://main/behavior/chat/send`。

**Server `forward_reply_to_agent` 用 `send/2` 不是 `cast/2`**:`Esr.Bridge.V1Prototype.Server.forward_reply_to_agent(bridge_id, text)` 用 `send(agent_pid, {:reply_received, text})` 投递,landed 在 Kind.Server 的 `handle_info`(per 上)。NOT `GenServer.cast`(会撞 Phase 1 `handle_cast({:esr_dispatch, _}, _)` 唯一 clause)。

**Application boot**(2b-step 1):
- esr_core Application children 加 `Esr.Entity.Agent.Supervisor`(DynamicSupervisor)— 给 bridge announce 时 spawn 用
- esr_plugin_chat Application start/2 时 **按序** spawn:
  1. `Esr.Entity.Session` at `session://main`(用 Supervisor.start_link with explicit ordering;不依赖默认 children list order)
  2. `Esr.Entity.User` at `user://admin`(**替换** Phase 1 admin_uri/admin_caps stub Kind 形态)
...
- Join dispatch 用 `:cast` 模式 — 若 Session 还在 `:not_ready`(Phase 1 reliability primitives,ReadyGate),`Esr.PendingDelivery` buffer until Session announce_ready 后 flush
...
**Controller 改造**(`apps/esr_web/lib/esr_web/controllers/cc_bridge_announce_controller.ex`):
- `announce/2` 改:除了 register 到 V1Prototype.Server 还触发 spawn `Esr.Entity.Agent` Kind(at agent_uri)
- `reply/2` 改:**不再调** `Esr.Bridge.V1Prototype.Server.record_reply`(这条 Phase 1 的状态机砍掉);改为 `forward_reply_to_agent(bridge_id, text)`,内部由 Server 找到 Agent Kind 然后发信
- `disconnect/2` 改:除了 unregister 还 terminate Agent Kind(through Server)

**Bridge Server 扩展**(`apps/esr_plugin_cc_bridge_v1_prototype/lib/esr/bridge/v1_prototype/server.ex`):
- `forward_reply_to_agent(bridge_id, text)` 新 API
- 内部维护 `bridge_id -> agent_pid` 映射,通过它找 Kind 发 `{:reply_received, text}` cast

**Agent Kind 内部 handle_info**:`{:reply_received, text}` → 构造 `%Esr.Message{sender: state.uri, body: %{text: text, attachments: []}, mentions: []}` → dispatch `session://main/behavior/chat/send` via Esr.Invocation.dispatch

**LV chat UI 重建**(`apps/esr_web_liveview/lib/esr_web_liveview/admin_live.ex` 大改):
...
- 老的 `Esr.Audit.stream_topic()`:Debug 区 audit log 仍订阅
...
- esr_core 18 模块 dispatch backbone
- esr_plugin_echo(Echo Kind + Behavior)— Phase 2 不退化
- esr_web_liveview AdminLive Phase 1b 形态(2c 大改之但保留 Debug 区)
- esr_plugin_cc_bridge_v1_prototype(MCP+channel bridge,SSE 流,reply 路径)
- `.mcp.json` 项目根 drop file 机制(Decision #87)
- `scripts/cc-bridge-attach.sh` + `.local.sh` 模式
- agent-browser 验证流程

## 当前 esr-ng 状态对照(Phase 2 新借鉴 / 反例)
...
- ❌ **CC channel 完整 plugin `esr_plugin_cc_channel`**— Phase 5。Phase 2 继续用 v1_prototype(但 controller reply 路径升级走 Agent Kind,这部分被 Phase 5 wholesale replace 时一并替换)
...
`apps/esr_plugin_cc_bridge_v1_prototype/` 整 plugin 保留 `_v1_prototype` 后缀,Phase 5 由 `esr_plugin_cc_channel` wholesale replace。Phase 2 在该 plugin 内的改动(controller reply 路径走 Agent Kind 等)Phase 5 替换时一并扔。
...
- **#90** Phase 2 Agent Kind 动态 spawn 机制(bridge announce → spawn,disconnect → terminate,URI from `ESR_AGENT_URI` env)
