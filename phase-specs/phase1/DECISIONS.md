...
### P1-D1 — Bridge↔ESR wire = stdio + JSON-RPC(Q1)

bridge↔CC 必须 stdio(channels 协议硬要求,不变式 #8)。bridge↔ESR 也用 stdio + JSON-RPC,ESR 用 OSProcess 风格拉起 bridge 子进程(Phase 1 临时用 `:erlexec.run/2` 直接调,标 `TODO Phase 5 替换为 OSProcess Behavior`)。
**理由**:1c(实施时改名 1b)是 `_v1_prototype`,Phase 5 wholesale replace。YAGNI — 不给注定扔的原型镀金接近 Phase 5 形态。

### P1-D2 — 不用 `use Esr.Kind` 宏,用单 server + behaviour callbacks(Q2)

每个 Kind 模块只声明 `@behaviour Esr.Kind` + 几个 callback 函数(`type_name/0`、`behaviors/0`、`persistence/0`、可选 `uri_from_args/1`)。整个系统**一个**共享 `Esr.Kind.Server` GenServer,Kind 实例由 `Esr.Kind.Server.start_link({kind_module, args})` 启动。
...
- 代价:所有 Kind 共享 `Esr.Kind.Server` module,**Kind 间隔离从 compile time 推到 runtime**
  - `Esr.Kind.Runtime.handle_dispatch/3` 必须 defensive 处理任何 Kind 传进来的 state shape
...
- **1a**:esr_core MVP + esr_web_liveview /admin(~600 LOC core + ~150 LOC plugin,SPEC §1a Deliverables 模块表逐项加起来),内部 6 个 PLAN step,每步 commit 作 fine-grained checkpoint
...
- 路径 1:`Phoenix.PubSub.broadcast(Esr.PubSub, "esr:audit:stream", {:audit_event, event})` 给 LV(view 渲染,§5.7.6 合法 broadcast)
- 路径 2:`GenServer.cast(Esr.Audit.Writer, {:write, event})` 给 SQLite(异步持久化,Decision #60)
...
### P1-D5 — admin 常量挂在 `Esr.Entity.User`,无独立 `Esr.Bootstrap` 模块(Q5,修正自初版 B)

Phase 1 的 `Esr.Entity.User` 是 stub Kind 模块,**包含 admin-specific 常量函数**:
```elixir
defmodule Esr.Entity.User do
  @behaviour Esr.Kind
  def type_name, do: :user
  def behaviors, do: []                # Phase 3d 加 [Esr.Behavior.Identity, ...]
  def persistence, do: {:snapshot, :on_change}
  def admin_uri, do: URI.parse("user://admin")
  def admin_caps, do: MapSet.new([%Esr.Capability{kind: :any, behavior: :any, instance: :any, ...}])
end
```
...
**初版 brainstorm 我推荐独立 `Esr.Bootstrap` 模块(B 选项),Allen 反问「admin 不就是 Esr.Entity.User 吗?」一针见血 — 我接受修正**。admin-specific 知识就该在 User Kind module 上,不需要 Bootstrap 命名空间。**不可 revoke 守门仍在 `Esr.Capability.revoke/2`(Decision #81 不动)**。
...
4. esr_plugin_echo + Application 接线 — **PLAN-内部 checkpoint:F1 直接 invoke 在 `mix test` 跑通**
5. esr_web_liveview plugin(/admin LiveView + stream + form + Echo 按钮)
...
### `Esr.Kind.Server` state 形状的细节

主体 3 个字段已定(`%{kind, uri, state}`),但 `state` 里 slice keys 的命名约定 — 用 atom(`:identity`)还是 module(`Esr.Behavior.Identity`)?
...
- 命名空间 (`:"Esr.Behavior.Chat"`) — 防 plugin 碰撞,序列化字符串更长
...
- **A**:每个 primitive 自己的 GenServer 持表(`Esr.ReadyGate.Server` 等 5 个 GenServer)
- **B**:一个共享 `Esr.Core.EtsOwner` GenServer 持所有 5 张表
- **C**:在 `Esr.Application.start/2` 直接 `:ets.new` 拥有

**原则**:B(共享 EtsOwner)— 减少 supervision tree 噪声 + 单点重启恢复所有表;读写都通过模块函数,GenServer 只负责 lifecycle。Phase 2+ 如某张表需要独立 backpressure(`Esr.Idempotency` 的 LRU prune),那张表升 A;其他保 B。
...
`Esr.Kind.Server.init/1` 依赖三张 ETS table(`:esr_kind_registry` / `:esr_ready_gate` / `:esr_behavior_registry`),**必须**先于第一个 Kind 实例 spawn 时 ready。Phase 1 SPEC §1a-step 4 写 "启动时 spawn 一个 default echo 实例" — 如果 `EtsOwner` 启动顺序晚于 `EchoApplication`,**100% boot 失败**(`ArgumentError: ETS table does not exist`,或 `Registry not_alive`)。

**Phase 1 Application children 推荐顺序**(`apps/esr_core/lib/esr_core/application.ex`):

```elixir
children = [
  Esr.Core.EtsOwner,              # ① 起 5 张 ETS table(ReadyGate/PendingDelivery/Idempotency/BehaviorRegistry/KindRegistry-aux)
  {Registry, keys: :unique, name: Esr.KindRegistry},  # ② stdlib Registry
  Esr.Audit.Writer,               # ③ 异步 batch flush GenServer
  Esr.Idempotency.Sweeper,        # ④ LRU prune
  Esr.DLQ.Sweeper,                # ⑤ DLQ evict
  EsrWeb.Endpoint                 # ⑥ HTTP/LV
  # ⑦ esr_plugin_echo 在自己的 mix.exs 依赖 esr_core,umbrella 自动后启动
]
```

**原则**:
- 任何 ETS-backed primitive **必须**在使用它的 GenServer 之前 ready
- umbrella `extra_applications` / `applications` 顺序保证 plugin app 后启动(因为 plugin depends on esr_core)
...
### Phase 0 既有的 `mix esr.check_invariants` 升级时机
...
→ Phase 1 step 1 commit **同时升级** `mix esr.check_invariants`:加上 VERIFICATION.md 1a-G2 列的 5 条 grep 检查;后续 step 完成时再追加适用的(step 2 加 #2 #3,step 3 加 #1 #6 #7)。
...
- **Patch B** — GLOSSARY.md Decision Log **#84**:"Phase 1 采用路径 B 不用宏 — `@behaviour Esr.Kind` + 共享 `Esr.Kind.Server`;property 等价 #66 但 means 不同;共享 Server 把 Kind 隔离从 compile time 推到 runtime;Phase 1 接受 trade-off 因为只有 Echo 一个业务 Kind;Phase 2+ 若 state shape 假设冲突再评估"
...
- Phase 1 不引入 `Esr.Bootstrap` 模块;admin 常量挂 `Esr.Entity.User`;`Esr.Capability.revoke/2` 守门(Decision #81 不动)— 大概 #86
