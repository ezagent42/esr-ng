# Your first 30 days on ESR

You've just joined the **esr-ng** project (Elixir Smart Routing). This doc is a calibrated path through the codebase + culture so you can land a meaningful contribution by end of month one without re-deriving everything from scratch.

ESR shipped v1 at Phase 7 closeout (2026-05-18, after 7 phases of brainstorm with Allen, the original architect). Allen has handed off — there's no Allen on call. The system has docs + CI gates + the `esr-developer` skill (your Claude Code agent will auto-load it) covering the architectural rules. Trust those + this doc.
...
```bash
git clone <repo-url> esr-ng
cd esr-ng
mix deps.get
mix esr.bootstrap          # one-command install (D7-5 / Decision #139)
```

`mix esr.bootstrap` runs `esr.home.init` + `deps.get` + `esr.home.adopt_db` + `ecto.create+migrate` + a health check. End state: DB at `~/.esr-ng/<profile>/db/esr_core.db` (NOT in repo tree), CLI token minted, ready to `mix phx.server`. If anything fails, the task prints which phase choked.
...
Best entry points:
- `apps/esr_core/lib/esr/invocation.ex` — the heart. Read `dispatch/1`.
- `apps/esr_core/lib/esr/kind/runtime.ex` — the 9-step dispatch flow + `authz_check/4`.
- `apps/esr_domain_chat/lib/esr/behavior/chat.ex` — most complex Behavior; well-commented.
- `apps/esr_plugin_feishu/` — fullest plugin reference.
- `apps/esr_plugin_echo/` — smallest plugin reference.
...
2. **Write a tiny plugin** following `docs/onboarding/adding-a-plugin.md`. Even an echo-style plugin. Install via `mix esr.plugin.install` against running phx.
3. **Add an invariant test** for an existing pattern you find under-tested. Look at `apps/esr_domain_chat/test/integration/workspace_isolation_test.exs` for the canonical pattern.
...
- **PR 40: `Esr.Entity.Agent.spawn/4` + slice workspace_uri + spawned_by fields** (M). Unlocks the orchestrator lineage tracking (PR 42 ships the cap shape; PR 40 fills in the data).
...
```bash
mix test --include slow apps/esr_domain_chat/test/integration/workspace_isolation_test.exs
mix test apps/esr_core/test/esr/capability_test.exs
mix test apps/esr_plugin_feishu/test/sidecar_orphan_reap_test.exs --include slow
mix test apps/esr_cli/test/integration/cli_lv_cap_parity_test.exs
mix test apps/esr_domain_chat/test/esr/behavior/chat_test.exs
mix test apps/esr_domain_identity/test/esr/entity/user_test.exs
...
- **"Add `mix esr.plugin.uninstall`"** — Kind lifecycle management for live instances is non-trivial (Decision #142 — defer).
...
ESR v1 = "production-grade session-template generator" + "self-sustaining for dev team without Allen as fallback." Phase 7 closeout (2026-05-18) is the v1 release boundary. Your job: keep the invariants green, ship features, build whatever Phase 8+ direction makes sense for the team.
