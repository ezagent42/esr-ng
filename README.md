...
4. **`CLAUDE.md`** — ESR 特有约定(补充 phx.new 生成的 `AGENTS.md`)。
...
```
apps/esr_core/   — 纯 OTP,不依赖 :phoenix 框架(可用 :phoenix_pubsub / :ecto)
apps/esr_web/    — Phoenix transport 层(Endpoint / Socket / Plug / LiveView)
...
mix esr.check_invariants       # 8 条硬不变式自查(Phase 0:skeleton no-op)
