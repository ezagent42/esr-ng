# ESR v0.3 架构评审 — Grill Findings

> **评审对象**: `ESR_ARCHITECTURE.md` v0.3（2026-05-14）
> **评审方法**: 通读 v0.3 全文 + 实测现有 ESR 代码库的等价模块行数 + 通读 6 份现有 ESR 事故复盘（`docs/notes/`）
> **写给谁**: v0.3 的架构师
> **重要前提**: 你看不到现有 ESR 代码。所以这份文档里每一条都配了**代码示例**和**功能解释**——你可以脱离 ESR 代码库、纯靠推理来验证我说的对不对。凡是"我猜你的想法"的地方都明确标了 **【揣测意图】**。凡是引用现有 ESR 代码量的地方都是我**实测**的，不是估的。
...
2. **现有 ESR 踩过的 5 个真实事故坑，v0.3 没设计进去**——而 v0.3 偏偏是个全 PubSub 架构，其中 3 个坑会原样复现
...
**步骤 12 — `Esr.Invocation.reply(ctx, result)`**。§12.5 的 reply 路由表有 7 个 case：

```elixir
def reply(%{reply: {:phoenix_channel, topic}}, result),
  do: EsrWeb.Endpoint.broadcast(topic, "reply", result)
def reply(%{reply: {:phoenix_pubsub, topic}}, result),
  do: Phoenix.PubSub.broadcast(Esr.PubSub, topic, {:esr_reply, result})
...
def reply(%{reply: {:caller_inbox, pid}}, result),
  do: send(pid, {:esr_reply, result})
def reply(%{reply: :ignore}, _result), do: :ok
```

光这个表，加上 dispatch 主流程（parse + lookup + call + 错误路径），70 行**几乎肯定撑不住，会撞到 90 的 cap**。

我实测了现有 ESR 里最接近 dispatch 的两个模块（`handler_router` + `handler`）一共 116 行——而且它们**只覆盖了"调用 Python worker"这一条窄路径**，不含 arg validation、不含 7-case reply 表。
...
**建议**: 要么把 `matcher.ex` 的 target 提到 ~85 / cap 110；要么——更符合 v0.3 自己"少发明，移到 plugin"的原则——**把整个 routing matcher 拆成独立 plugin `esr_routing_matcher`**。它不是 `esr_core` 的必需品：core 只需要 RoutingRegistry 能存 `(matcher_data, receivers)`，至于 matcher 怎么求值，完全可以是 plugin 的事。

## 1.4 `kind.ex` ~40 LOC（cap 60）—— 低估

【揣测意图】你可能觉得 `use Esr.Kind` 是个薄薄的注册宏。
...
defmodule Esr.Entity.Agent do
  use Esr.Kind,
    behaviors: [Esr.Behavior.Identity, Esr.Behavior.Movable, Esr.Behavior.Spawnable],
...
我无法给你一个"现有 ESR 等价物"的精确数字，因为现有 ESR 没有统一的 `use Esr.Kind` 宏（它的 Kind 类型是手写 GenServer）。但正因为现在是手写的，每个 Kind GenServer 都是几百行——这个宏要"替所有 Kind 把那几百行的公共部分收进来"，它不可能是 40 行。
...
实测：现有 ESR 有 9 个 `registry.ex`，一共 **1722 行**。我看了其中最大的两个，它们**不只是存储**：
...
**真相是**: `esr_core` 的 routing_registry.ex 确实只有 ~60 行（存储底座），但那 ~1000+ 行的 routing **逻辑没有消失，只是从"散落在 core"搬到了"各 owner plugin 内部"**。这其实**符合** v0.3 的设计哲学（§5.4.6"core 不预定义任何 table"），是好事——但文档应该**诚实地说出来**："core 60 行是存储底座；prefix 匹配、overlay、attach/detach 这类逻辑由 owner plugin 自己实现"。否则读者会低估总工作量。
...
**建议**: 重新校准。我的诚实估算是 esr_core 真实落地在 **650-800 行**之间。要么接受这个数字（把 475/595 改成 ~700，红线提到 900），要么把 matcher 拆成 plugin（省 ~85 行）、把 arg validator 也算清楚。关键是让"红线"重新变得有意义。

---

# 二、现有 ESR 踩过的坑，v0.3 没设计进去

现有 ESR 在 `docs/notes/` 里有一批事故复盘。我读了其中 6 份。**v0.3 已经学到的坑**（值得肯定）：

- ✅ erlexec 子进程孤儿生命周期 —— §6.3 明确写了"Kind GenServer 是 erlexec port owner、`terminate/2` 必须 kill、`ESR_SPAWN_TOKEN`、开机扫 `/tmp/esr-worker-*.pid`"。这一条现有 ESR 出过"8 倍孤儿"事故（一条消息被回复 8 次），v0.3 把教训完整内化了。
- ✅ Capability 改成 struct（§3.4），而不是字符串。现有 ESR 出过"spec 写 `cap.session.create`、代码要 `prefix:name/perm`"的格式不匹配 bug。v0.3 用 struct 从根上消除了这个坑。
- ✅ Feishu WebSocket 留在 Python（§12.4.A）。现有 ESR 明确记录过"别想把 Feishu WS 搬进 Elixir"——lark SDK 是 Python 生态。v0.3 尊重了这一点。

下面是**没学到的 5 个**，按严重度排。每条都给：现有 ESR 的真实事故 → 代码示例 → 揣测你的想法 → 为什么还是个坑 → 建议。

## 2.1 🔴 P1 — PubSub「先发后订」静默丢消息

**现有 ESR 的真实事故**（`docs/notes/cc-mcp-pubsub-race.md`）：
...
# 立刻路由第一条消息
Phoenix.PubSub.broadcast(Esr.PubSub, "#{session_uri}:inbound", first_message)
#                                     ↑
...
**建议**: 在 §5.5 或 Appendix A 加一个"投递保证"小节。现有 ESR 事故文档给了三个方案，最干净的是 **B（buffer + flush on ready）**：
...
def deliver(receiver_uri, message) do
  case Esr.KindRegistry.lookup(receiver_uri) do
    ...
    :error     -> Esr.PendingDelivery.buffer(receiver_uri, message)
  end
end

# actor ready 后主动 flush
def handle_continue(:ready, state) do
  Esr.PendingDelivery.flush(state.uri) |> Enum.each(&handle_inbound/1)
  {:noreply, state}
end
```
...
**现有 ESR 的真实事故**（`docs/notes/mcp-transport-orphan-session-hazard.md`）：
...
def connect(%{"token" => token, "cc_instance" => instance_id}, socket, _info) do
  case verify_token(token, instance_id) do
    {:ok, agent_uri} ->
      Esr.RoutingRegistry.put(CCInstanceConnection, instance_id, agent_uri)
      #                       ↑
...
**现有 ESR 的约束**（`docs/notes/system-invariants.md` 的 invariant I5）：

现有 ESR 有一条**硬 CI gate**：routing 层的代码**不准**出现 `other -> Logger.warning + drop` 这种 catch-all。理由很简单——静默丢消息是不可观测的 bug，必须从代码层面禁掉。
...
**【揣测意图】** 你可能觉得："匹配不到 rule 的消息就是 no-op，没问题。" —— 在一个**纯路由层**这么想是合理的。但 ESR 是个 **chat 系统**：用户发的一条消息**到达了零个 receiver**，这本身就是 bug——用户期待"有人收到"。返回 `[]` 跟"正常工作"在代码上无法区分，所以这个 bug 永远不会被发现。
...
def route(session_uri, message) do
  ...
  case receivers do
    [] ->
      :telemetry.execute([:esr, :routing, :unroutable], %{}, %{session: session_uri, message: message})
      Esr.DeadLetter.put(:unroutable, message)
      []
    rs -> rs
  end
end
...
**背景**：Feishu（以及绝大多数 webhook 提供方）在没有快速收到 200 响应时**会重试投递**。同一条 inbound 消息可能到达 2-3 次。现有 ESR 在它的 actor 里用一个 idempotency key 做去重（bounded MapSet，记最近见过的 key）。
...
case ctx[:idempotency_key] do
  nil -> :proceed
  key ->
    if Esr.Idempotency.seen?(key),
      do: {:ok, :duplicate_ignored},          # 重试到达，直接返回，不进 Kind
      else: Esr.Idempotency.record(key)        # 第一次见，记下，继续
end
```

`Esr.Idempotency` 是个 bounded 的 ETS 表（记最近 N 个 key，LRU evict）——~20 行，可以是 core 的一部分，也可以是 `esr_behavior_idempotency` plugin。

## 2.5 🟡 P3 — SQLite 缓存模块名的 staleness

**现有 ESR 的真实事故**（`docs/notes/refactor-lessons.md` §三-2）：

现有 ESR 的运行时状态文件里缓存了**字符串形式的模块名**（比如 `command_module: "Esr.Admin.Commands.Session.End"`）。当你 rename 那个模块后，磁盘上缓存的字符串还是旧的 → daemon 启动时按旧名字找模块 → `unknown_module` → 启动失败级联。
...
CREATE TABLE kind_snapshots (
  ...
  kind_module  TEXT NOT NULL,     -- ← 存的是 "Elixir.Esr.Entity.Agent" 这种字符串
  state        TEXT NOT NULL,
  ...
);
```
...
**【揣测意图】** 你可能觉得："esr-ng 是全新的库，不会有现有 ESR 那种大规模 rename。" —— **部分对**，新库的 rename 压力确实小很多。但在 v0 早期开发阶段，Behavior / Kind 模块**一定会被改名**（这就是开发）。一个 keyed 到旧模块名的 snapshot 行，rename 后就 rehydrate 失败。这个结构性 hazard 跟着"用字符串存模块引用"这个设计回来了——只是存储介质从 YAML 换成了 SQLite。

**为什么标 P3 不是 P1**：因为 esr-ng 是 greenfield、rename 频率低，而且影响面是"开发期偶尔炸一下"，不是"生产事故"。但值得在 §10 提一句。
...
现有 ESR 的 workspace 有这些操作（我实测，现有 `commands/workspace/` 有 12 个命令模块）：`add-folder`、`bind-chat`、`import-repo`、`list-registered-repos`、`describe`、`set-default`……
...
**给你的具体建议**：v0.3 不需要删掉 Workspace 概念，需要的是 **§3.5 之后加一段，明确 `Resource` Kind 的"薄"形态**——一个 Resource 可以几乎没有自己的运行时状态，它存在的意义是"给操作它的 Behavior 一个 URL 可寻址的挂载点"。然后把 Workspace 当成这个薄形态的**示范例子**，列出它的标准 Behavior 集（`Esr.Behavior.WorkspaceFolders`、`Esr.Behavior.WorkspaceBindings` 等）。

这样你既保住了"workspace 不是重量级 actor"的正确直觉，又没有在统一模型上开口子。

## 3.2 Plugin 运行时配置 —— 可能只是没说，不是没有

**【揣测意图】** §9.4 的 plugin discovery 用 `Application.spec/2` 读 `:env` 里的 `:esr_kinds`。我猜你可能觉得"plugin 配置就是 `:env` 块"。

但 `:env` 是**编译期静态**的。现有 ESR 的 plugin 有**运行时可变**的配置——比如 Feishu plugin 要配"用哪个 Feishu app 的 credentials"、"哪些 chat 绑到哪些 workspace"，这些是运维在系统运行时会改的，不是编译进 `:env` 的。
...
【揣测意图】你可能觉得这隐含在 Kind 子类里（Session 显然是多实例、按需建）。对 Session 是清楚的。但 `Esr.Entity.Agent`——是每个 session 一个 agent 实例，还是一个 agent 实例跨 session 复用？v0.3 没说。这个不紧急（P3），但开始写 `Esr.Kind.Template` 的 `instantiate/2` 时会撞到。
...
| 🟠 P2 | Appendix A 加 step 2.7 幂等去重；加 `Esr.Idempotency` 模块或 plugin | Appendix A / §14 |
...
**最重要的两条是 🔴 P1**。它们是现有 ESR 的真实生产事故，而 v0.3 是个**全 PubSub 架构**——这两个坑不设计进去，会原样复现，而且都是"静默丢消息、无报错"的那种最难查的 bug。
...
- **v0.3 全文**：2170 行，通读。
- **LOC 实测**：现有 ESR 代码库里，每个 v0.3 core 模块的最接近等价物，用 `wc -l` 实测。具体数字在正文里标了"实测"。我没有猜任何一个数字——猜不出来的地方（比如 `kind.ex` 没有现成等价物）我明说了"无法给精确对照"，只做功能推理。
