...
本文件 supplements `phx.new` 生成的 `@AGENTS.md`(Phoenix/Elixir LLM 常见错误修正,由 Phoenix.new agent 提炼)。**先读 AGENTS.md 拿 Phoenix idioms,再读本文件拿 ESR 特有约定**。
...
- ESR 跟 typical Phoenix app 的两条核心差异是什么?
- 8 条硬不变式是哪些?
...
1. **inbound 永远走 `Esr.Invocation.dispatch/1`** — 不允许裸 `Phoenix.PubSub.broadcast` 到 inbound topic。Phoenix.PubSub 不 buffer 没订阅者的 topic,裸 broadcast 在 register→subscribe 窗口会丢消息(事故 2.1 根因)。Grep:`PubSub.broadcast` 出现在 inbound 路径 = bug
2. **`use Esr.Kind` 生命周期严格 register→subscribe→announce_ready** — plugin 作者无法绕过。手写 `init/1` 不调宏 = bug
...
6. **Audit 异步 cast,不阻塞 invoke** — `:telemetry` handler 只 `GenServer.cast` 到 `Esr.Audit.Writer`,不直接写 SQLite
7. **零匹配路由必须 telemetry + DLQ unroutable,不能静默** — 默认静默 = ESR 是 router 不是 req/resp app 的根问题
...
# 不变式 #2 反例
grep -rn "def init/1" lib/ | grep -v "use Esr.Kind"   # 手写 init 跳过宏 = bug
```
...
- `esr_core` target ~870 LOC,red line 1100
- 每模块有 cap(详见 §14)
- 写完后 `wc -l lib/esr_core/**/*.ex` 核对,超 cap 触发设计 review
...
ESR 跟外部世界有很多同名概念,**用错术语会让架构理解漂移**。常见的:

| 词 | ESR 意义 | 不要混淆 |
|---|---|---|
| **channel** | Claude Code Channel(MCP 协议) | Phoenix.Channel(WS 抽象) |
| **session** | ESR Session(routing context owner) | Phoenix session(cookie/web session) |
| **registry** | KindRegistry(URI→pid)或 RoutingRegistry(routing rules) | Elixir Registry(底层 module) |
| **behavior** | Esr.Behavior(action 处理者) | Elixir behaviour(callback 契约) |
| **template** | Template Class(模块级)或 Template Instance(运行时 Resource) | Phoenix template(.heex 文件) |
| **plugin** | OTP app 形式的 ESR 扩展 | Mix.Project plugin(完全不同) |
| **dispatch** | `Esr.Invocation.dispatch/1`(消息分发) | Phoenix.Router.dispatch(HTTP 路由) |
...
# Phase 0 后才有:
# (Phase 0 brainstorm 时定义自定义 mix tasks,例如 mix esr.check_invariants)
```
...
## ESR 是 router 不是 req/resp app(读这一节之前你应该已经读过 ARCHITECTURE.md §1.2)
...
- **重复 inbound(webhook 重试)会怎样?**
  - `ctx.idempotency_key` + `Esr.Idempotency.seen?/1` 检查
  - v0 语义:**收到即记**(失败也算 seen),失败走 DLQ 兜底

每写一个路由/投递点,**问"这里失败了谁会知道"**——这是 ESR 比 typical Phoenix app 多出来的认知负担,没有别的办法。

---

## 关于 grill 文化

ESR 的 ARCHITECTURE.md 是 Allen 跟工程师做了 4 轮 grill 闭环写出来的,每条决策都有论证。实施期你可能会:
...
ESR 不是一个普通 Phoenix app,本文件 + ARCHITECTURE.md + GLOSSARY.md 三件套保证你不犯典型错误。如果有疑问,**问 Allen,不要假设**。
